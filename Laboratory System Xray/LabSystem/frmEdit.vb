...
Public Class frmEdit
    ...
    Dim Rmks, StartSno, strXInfo, Inv_String As String
    Dim strQty As Integer = 0
    Dim Inv_Stock As Integer = 0
    Dim Inv_Code As String = ""
    ...
    Private Sub txtCode_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtCode.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            Inv_String = lblXRay.ToString.ToUpper
            ' Check if Test already in the receipt table
            Dim CommandText As String = "SELECT * FROM RECEIPT where tcode = '" & txtCode.Text.Trim & "' AND TCASENO = " & Val(txtRecNo.Text.Trim)
            ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, CommandText, "RECEIPT", Connection)
            SqlDataAdapt.Fill(SqlDataSet, "RECEIPT")
            If SqlDataSet.Tables("RECEIPT").Rows.Count > 0 Then
            ...
            Try
                ...
                If objDataTable.Rows.Count > 0 Then
                    ...
                    If Inv_String.IndexOf("XRAY") > -1 Then
                        Inv_String = lblXRay.Text.Trim.ToUpper
                        ...
                        Dim NewRecCommand As String = "INSERT INTO INV_TRANS VALUES ('" & _
                                            ...
                                            Val(strQty) & "," & _
                                            Val(Inv_Stock) & "," & _
                                            Val(Inv_Stock) - Val(strQty) & "," & _
                                            "'ISSUE')"
                        Execute_DatabaseTable_Query(SqlDataAdapt, NewRecCommand, Connection)

                        Dim Updt_Query As String = "UPDATE INVENTORY SET INV_IN_HAND = (INV_IN_HAND - " & Val(strQty) & ") " & _
                    ...
                    Dim EditCommandText As String = "INSERT INTO RECEIPT VALUES (" & _
                                                    Val(txtRecNo.Text) & ",'" & _
                                                    strTCode & "','" & _
                                                    strTname & "','" & _
                                                    strCatg & "','" & _
                                                    Inv_String.Trim & "'," & Val(strQty) & ",'" & _
                    ...
                    Connection.Close()
                    SqlDataAdapt.Dispose()
                    Call ShowGridData("SELECT * FROM RECEIPT", "RECEIPT", txtRecNo.Text)
                    txtCode.Clear()
    ...
    Private Sub cmdDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDelete.Click
        Dim DelString, DelTname, DelQty, DelInvItem As String
        DelInvItem = ""
        DelQty = ""
        DelTname = ""
        ...
        '// Select Deleted Test Entry Info
        Dim strSql As String = "Select * from RECEIPT WHERE TCODE = '" & DelString.Trim & "' AND TCASENO = " & Val(txtRecNo.Text)
        ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, strSql, "RECEIPT", Connection)
        Dim DelReader As SqlDataReader = SqlDataAdapt.SelectCommand.ExecuteReader
        Try
            While DelReader.Read
                DelQty = DelReader.Item("tqty").ToString
                DelInvItem = DelReader.Item("tqtydesc").ToString
                DelTname = DelReader.Item("tname").ToString
            End While
            If Not DelReader Is Nothing Then DelReader.Close()

            '// Update Inventory if Delete Entry Confirmed
            ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, "SELECT * FROM INVENTORY WHERE INV_DESCRIPTION LIKE '%" & DelInvItem.Trim & "%'", "INVENTORY", Connection)
            cpReader = SqlDataAdapt.SelectCommand.ExecuteReader
            While cpReader.Read
                Inv_Stock = Val(cpReader.Item("INV_IN_HAND").ToString)
                Inv_Code = cpReader.Item("INV_CODE")
            ...
            If DelTname.IndexOf("XRAY") > -1 Then
                Dim NewRecCommand As String = "INSERT INTO INV_TRANS VALUES ('" & _
                                                Format(DateTime.Parse(DateTime.Now.ToShortDateString.ToString), "MM/dd/yyyy") & "','" & _
                                                "XR-" & txtRecNo.Text.Trim & "','" & _
                                                Inv_Code.Trim & "','" & _
                                                DelInvItem.Trim & "','" & _
                                                "Cancelled by Patient" & "'," & _
                                                Val(DelQty) & "," & _
                                                "0," & _
                                                Val(Inv_Stock) & "," & _
                                                Val(Inv_Stock) + Val(DelQty) & "," & _
                                                "'ADD')"
                Execute_DatabaseTable_Query(SqlDataAdapt, NewRecCommand, Connection)

                Dim Updt_Query As String = "UPDATE INVENTORY SET INV_IN_HAND = (INV_IN_HAND + " & Val(DelQty.Trim) & ") " & _
                                                   "WHERE INV_CODE = '" & Inv_Code.Trim & "'"
                Execute_DatabaseTable_Query(SqlDataAdapt, Updt_Query, Connection)
            End If
            '// Delete Selected Record from Receipt
            Dim CommandText As String = "DELETE FROM RECEIPT WHERE TCODE = '" & DelString.Trim & "' AND TCASENO = " & Val(txtRecNo.Text)
            SqlDataAdapt.Dispose()
            SqlDataSet.Clear()
    ...
    Private Sub cmdCancel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCancel.Click
        ...
        If Val(strDiff) <> 0 And Val(lblTotal.Text) > 0 Then
            SqlString = "UPDATE PATIENT SET p_delete = " & Val(lblTotal.Text) & ", p_del_paid = " & Val(txtPaid.Text) & ", p_del_dues = " & Val(lblDues.Text) & ", p_del_disc = " & Val(txtDiscount.Text) & "," & _
    ...
    Private Sub cmdComp_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdComp.Click
        ...
        Dim strDiff As String = DateDiff(DateInterval.Day, Date.Today, strDate)
        If Val(strDiff) <> 0 And Val(lblTotal.Text) > 0 Then
            SqlString = "UPDATE PATIENT SET p_compliment = " & Val(lblTotal.Text) & ", p_com_paid = " & Val(txtPaid.Text) & ", p_com_dues = " & Val(lblDues.Text) & ", p_com_disc = " & Val(txtDiscount.Text) & "," & _
        ...
        If Val(strDiff) = 0 And Val(lblTotal.Text) > 0 Or Val(strDiff) = 0 Then
            SqlString = "UPDATE PATIENT SET p_compliment = " & Val(lblTotal.Text) & ", p_paid = 0, p_dues = 0, p_discount = 0, p_other = 0, p_del_ok = 'Complimentary', p_del_operator = '" & SysOperator.Trim & "', p_del_date = '" & Format(Now(), "MM/dd/yyyy") & "'," & _
                                            " p_del_time = '" & Format(Now(), "hh:mm tt") & "', p_comp_on = '" & Format(Now(), "MM/dd/yyyy") & "', p_comp_by = '" & SysOperator.Trim & "', p_edit_opr = '" & SysOperator.Trim & "'," & _
                                            " p_edit_on = '" & Format(Now(), "MM/dd/yyyy") & "', p_reason = '" & StrConv(Reason.Trim, VbStrConv.ProperCase) & "' WHERE p_num = " & Val(txtRecNo.Text)
        End If
    ...
    Private Sub txtRefer_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        isChanged = True
    End Sub
    ...
    Private Sub txtName_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtName.KeyDown
        If e.KeyCode = Keys.Enter Then txtName.Text = StrConv(txtName.Text, VbStrConv.ProperCase) : cmbCP.Focus()
    End Sub
    ...
    Private Sub cmbCP_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles cmbCP.KeyDown
        If e.KeyCode = Keys.Enter Then cmbXray.Focus()
    End Sub
