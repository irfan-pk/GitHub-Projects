...
Public Class AccountJournalBook
...
#Region "GLOBAL VARIABLES"
    ...
    Dim CheckTransAdapter As New SqlDataAdapter()
    Dim DailyTransAdapter As New SqlDataAdapter()
    Dim DuesDataAdapter As New SqlDataAdapter()
    Dim DeleteDataAdapter As New SqlDataAdapter()
    Dim DeletePayAdapter As New SqlDataAdapter()
    Dim CompDataAdapter As New SqlDataAdapter()
    Dim CompPayAdapter As New SqlDataAdapter()
...
#Region "FORM DESIGN"
    Private Sub HdrPicBox_Paint(ByVal sender As Object, ByVal e As System.Windows.Forms.PaintEventArgs) Handles HdrPicBox.Paint
        ...
        'Dim lgb As New LinearGradientBrush(rect, Color.Gray, Color.DarkGray, LinearGradientMode.Vertical)
        'g.FillRectangle(lgb, 0, 0, HdrPicBox.Width, HdrPicBox.Height)
        'g.DrawString("General Journal Entries", fnt, Brushes.White, 8, 8)
...
#Region "FORM EVENTS"
    Private Sub cmdClose_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdClose.Click
        ProcessTransaction("DELETE FROM GEN_TEMP")
        Me.Close()
    End Sub
    ...
    Private Sub frmJournal_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Execute_DatabaseTable_Query(SqlDataAdapter, "select max(ledg_date) as MaxDate from ledgers", Connection)
        ...
        ProcessTransaction("DELETE FROM GEN_TEMP")
        Connection.Close()
        txtTranID.Text = GenerateMax_ID("GENERAL_JOURNAL", "GEN_TRAN_ID")
        Connection.Close()
        SortID = 0
        TransActive = False
        TransRetrieve = False
...
#End Region
    ...
    Private Sub txtDebit_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtDebit.KeyDown
        If e.KeyCode = Keys.Enter Then
            If txtVoucher.Text.Trim = String.Empty And txtDescription.Text = String.Empty Then MsgBox("Doc # and Description should not be blank.", MsgBoxStyle.Critical, My.Application.Info.ProductName) : ProcessTabKey(False) : Exit Sub
            If Val(txtDebit.Text.Trim) = 0 Then Exit Sub
            gNarration = StrConv(txtDescription.Text.Trim, VbStrConv.ProperCase)
            Call DebitTransaction(txtAccount.Text.Trim, "DR", Val(txtDebit.Text.Trim), gNarration.Trim, StrConv(txtVoucher.Text.Trim, VbStrConv.ProperCase))
        End If
    End Sub
    ...
    Private Sub txtCredit_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtCredit.KeyDown
        If e.KeyCode = Keys.Enter Then
            If txtVoucher.Text.Trim = String.Empty And txtDescription.Text = String.Empty Then MsgBox("Doc # and Description should not be blank.", MsgBoxStyle.Critical, My.Application.Info.ProductName) : ProcessTabKey(False) : Exit Sub
            If Val(txtCredit.Text.Trim) = 0 Then Exit Sub
            gNarration = StrConv(txtDescription.Text.Trim, VbStrConv.ProperCase)
            Call CreditTransaction(txtAccount.Text.Trim, "CR", Val(txtCredit.Text.Trim), gNarration.Trim, StrConv(txtVoucher.Text.Trim, VbStrConv.ProperCase))
        End If
    End Sub
...
#Region "JOURNAL ENTRIES PROCEDURES"
    ...
    Private Sub Show_Journal_Transactions()
        ...
        ExecuteSqlQuery(SqlDataAdapter, Me.TransactionDataSet, "SELECT * FROM GEN_TEMP ORDER BY GEN_TRAN_ID, GEN_SORT", "GEN_TEMP", Connection)
    ...
    Private Sub cmdGetTrans_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdGetTrans.Click
        ...
        If cmdGetTrans.Text = "D&elete All" Then
            Execute_DatabaseTable_Query(SqlDataAdapter, "DELETE FROM GEN_TEMP", Connection)
            Connection.Close()
        ...
        Check_Trans_Date = "SELECT * FROM GENERAL_JOURNAL WHERE (GEN_DATE = CONVERT(DATETIME,'" & Format(DateTime.Parse(dtpTransaction.Value.ToShortDateString.ToString), "yyyy-MM-dd") & " 00:00:00', 102)) AND (GEN_SUB_ACC = 'LAB RECEIPTS')"
        Execute_DatabaseTable_Query(CheckTransAdapter, Check_Trans_Date, Connection)
        Dim CHECK_TRANS As SqlDataReader = CheckTransAdapter.SelectCommand.ExecuteReader
        While CHECK_TRANS.Read
            CHK_STRING = CHECK_TRANS.Item("GEN_SUB_ACC").ToString.Trim.ToUpper.Trim
        End While
        ...
        If TransRetrieve = True Then
            MsgBox("Transaction already posted for this date.", MsgBoxStyle.Information, My.Application.Info.ProductName)
            TransRetrieve = False
            CHK_STRING = ""
            Exit Sub
        End If
        ...
        ExecuteSqlQuery(SqlDataAdapter, SqlDataSet, Transaction_Check_Text, "PATIENT", Connection)
        If SqlDataSet.Tables("PATIENT").Rows.Count = 0 Then MsgBox("Check Data in Table for Journal Entries", MsgBoxStyle.Information, My.Application.Info.ProductName) : Me.Cursor = Cursors.Default : Exit Sub
        Connection.Close()
        ...
        ExecuteSqlQuery(DailyTransAdapter, SqlDataSet, Daily_Transaction_Query, "PATIENT", Connection)
        Execute_Trans_Reader = DailyTransAdapter.SelectCommand.ExecuteReader
        ...
        DailyVoucher = "D-" & Format(DateTime.Parse(dtpTransaction.Value.ToShortDateString.ToString), "ddMMyyyy")
        gNarration = StrConv("Daily Gross Collection from ", VbStrConv.ProperCase) & strCp.ToUpper

        If Val(wNet) > 0 Then Call DebitTransaction("Cash", "DR", wNet, "Daily Collection of Lab.", DailyVoucher)
        If Val(wDisc) > 0 Then Call DebitTransaction("Lab Discount", "DR", wDisc, "Daily Discount of Lab.", DailyVoucher)
        If Val(wDues) > 0 Then Call DebitTransaction("Lab Dues", "DR", wDues, "Receiveable Amount of Lab.", DailyVoucher)
        If Val(wCompl) > 0 Then Call DebitTransaction("Complimentary", "DR", wCompl, "Daily Complimentary Cases Amount.", DailyVoucher)
        If Val(wDelAdj) > 0 Then Call DebitTransaction("Cancelled Cases", "DR", wDelAdj, "Daily Cancelled Cases Amount.", DailyVoucher)
        If Val(wGross) > 0 Then Call CreditTransaction("Lab Receipts", "CR", wGross, "Daily Gross Receipts of Lab.", DailyVoucher)
        ...
        ExecuteSqlQuery(DuesDataAdapter, SqlDataSet, Transaction_Check_Text, "PATIENT", Connection)
        Execute_Trans_Reader = DuesDataAdapter.SelectCommand.ExecuteReader
        While Execute_Trans_Reader.Read
        ...
        If wNet > 0 Then
            DebitTransaction("Cash", "DR", wNet, "Cash Received from dues debited to " & StrConv(strCp.Trim, VbStrConv.ProperCase), DailyVoucher)
            CreditTransaction("Lab Dues", "CR", wNet, "Lab Dues Credited to " & StrConv(strCp.Trim, VbStrConv.ProperCase), DailyVoucher)
        End If
        ...
        ExecuteSqlQuery(DeleteDataAdapter, SqlDataSet, Transaction_Check_Text, "PATIENT", Connection)
        Execute_Trans_Reader = DeleteDataAdapter.SelectCommand.ExecuteReader
        ...
        ExecuteSqlQuery(DeletePayAdapter, SqlDataSet, Transaction_Check_Text, "PATIENT", Connection)
        Execute_Trans_Reader = DeletePayAdapter.SelectCommand.ExecuteReader
        While Execute_Trans_Reader.Read
        ...
        If wNet > 0 Then DebitTransaction("Cancelled Cases", "DR", wNet, gNarration, DailyVoucher)
        If wDisc > 0 Then CreditTransaction("Lab Discount", "CR", wDisc, gNarration, DailyVoucher)
        If wDues > 0 Then CreditTransaction("Lab Dues", "CR", wDues, gNarration, DailyVoucher)
        If wDelAdj > 0 Then CreditTransaction("Cash", "CR", wDelAdj, gNarration, DailyVoucher)
        ...
        ExecuteSqlQuery(CompDataAdapter, SqlDataSet, Transaction_Check_Text, "PATIENT", Connection)
        Execute_Trans_Reader = CompDataAdapter.SelectCommand.ExecuteReader
        ...
        ExecuteSqlQuery(CompPayAdapter, SqlDataSet, Transaction_Check_Text, "PATIENT", Connection)
        Execute_Trans_Reader = CompPayAdapter.SelectCommand.ExecuteReader
        While Execute_Trans_Reader.Read
            strCp = Execute_Trans_Reader.Item("P_CP").ToString.Trim
        ...
        If wNet > 0 Then DebitTransaction("Complimentary", "DR", wNet, gNarration, DailyVoucher)
        If wDisc > 0 Then CreditTransaction("Lab Discount", "CR", wDisc, gNarration, DailyVoucher)
        If wDues > 0 Then CreditTransaction("Lab Dues", "CR", wDues, gNarration, DailyVoucher)
        If wDelAdj > 0 Then CreditTransaction("Cash", "CR", wDelAdj, gNarration, DailyVoucher)
    ...
    Private Sub cmdPost_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPost.Click
        ...
        If Val(txtTotDebit.Text) = Val(txtTotCredit.Text) Then
            Me.Cursor = Cursors.WaitCursor
            Dim Post_Transactions As String = "INSERT INTO GENERAL_JOURNAL " & _
                                              "SELECT  *  from GEN_TEMP " & _
                                              "WHERE GEN_F_YEAR = '" & FinancialYear.Trim.ToUpper & "'"
            ProcessTransaction(Post_Transactions)
            ProcessTransaction("DELETE FROM GEN_TEMP")
            txtTranID.Text = GenerateMax_ID("GENERAL_JOURNAL", "GEN_TRAN_ID")
            Call Show_Journal_Transactions()
            txtTotDebit.Clear()
            ...
            txtAcc_Id.Focus()
        Else
            MsgBox("Transactions cannot be recorded with difference. please check transactions.", MsgBoxStyle.Information, My.Application.Info.ProductName)
        End If
        'If AccountLedgers.Visible = True Then
...
#End Region
    ...
    Private Sub cmdDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDelete.Click
        ...
        ExecuteSqlQuery(SqlDataAdapter, SqlDataSet, "SELECT * FROM GEN_TEMP WHERE GEN_TRAN_ID =" & Val(delTranID), "GEN_TEMP", Connection)
        ...
        ProcessTransaction("DELETE FROM GEN_TEMP WHERE GEN_TRAN_ID =" & Val(delTranID))
        Call ClearTextBoxes()
        Call Show_Journal_Transactions()
    End Sub
    ...
    Private Sub txtAcc_Id_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtAcc_Id.KeyDown
        If e.KeyCode = Keys.Enter Then
            Call Get_Account_Parameters(Val(txtAcc_Id.Text.Trim))
            lblAccHead.Text = gblAccHead
            txtAccount.Text = gblAccName
            ProcessTabKey(True)
        End If
    ...
    Private Sub txtVoucher_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtVoucher.LostFocus
        txtVoucher.Text = StrConv(txtVoucher.Text.Trim, VbStrConv.ProperCase)
    End Sub
End Class
