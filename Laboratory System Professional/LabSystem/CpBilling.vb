    ...
    Private Sub cmdPaid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPaid.Click
        ...
        If lblStatus.Text.Trim = "BILLED" Then
            Dim CommandText As String = "UPDATE PATIENT SET P_DEL_OK = 'Yes',p_paid = p_dues, p_due_rcv = p_dues, p_dues = 0, p_due_rcv_operator = '" & cmbOper.Text.ToUpper.Trim & "'," & _
                                        " p_due_rcv_date = '" & Format(DateTime.Parse(dtpPayDate.Value.ToShortDateString), "yyyy-MM-dd") & "', p_del_date = '" & Format(DateTime.Parse(dtpPayDate.Value.ToShortDateString), "yyyy-MM-dd") & "', p_del_time = '" & Format(DateTime.Parse(TimeOfDay.ToShortTimeString), "hh:mm tt") & "', p_del_operator = '" & cmbOper.Text.ToUpper.Trim & "'" & _
                                        " WHERE (p_date_sample >= CONVERT(DATETIME,'" & Format(fromDate, "yyyy-MM-dd") & " 00:00:00',102)" & _
                                        " AND p_date_sample <= CONVERT(DATETIME,'" & Format(toDate, "yyyy-MM-dd") & " 00:00:00',102)) AND (P_CP = '" & _
