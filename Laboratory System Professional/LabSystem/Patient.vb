...
Public Class Patient
    ...
    Dim StartID, StartSno, Rmks As String
...
#Region "KEY EVENTS"
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            Try
                ...
                Try
                    ...
                    'skip if cross match test detect
                    If strTname.IndexOf("CROSS") > -1 Then
                        MsgBox("Plz use Cross Match Entry Form.", MsgBoxStyle.Information, My.Application.Info.ProductName)
                        Exit Sub
                    End If
                    ...
                    If Val(strRptLvl) > 1 Then
                        dtResult = Format(DateTime.Parse(DateAdd(DateInterval.Day, Val(strRptLvl.Trim), Now)), "MM/dd/yyyy")
                    Else
                    ...
                    TestItem.SubItems.Add(dtResult.ToString)
    ...
    Private Sub txSRefer_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSRefer.GotFocus
        txtSRefer.SelectAll()
    End Sub
    ...
    Private Sub Patient_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
        If e.KeyCode = Keys.F4 Then
            gblFind = txtRecNo.Text.Trim
            If Len(gblFind) > 0 Then
                gblReportOption = "RECEIPT"
                gblReportName = ".\" & "Reports\LabSlip.rpt"
                StatReports.ShowDialog(Me)
            End If
        End If
    End Sub
#End Region
    ...
    Private Sub frmPatient_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            While cpReader.Read
                Me.cmbCollPoint.Items.Add(cpReader.Item("CP_NAME"))
            End While
    ...
    Private Sub cmdNew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdNew.Click
        ...
        Try
            ...
            Me.lblSequence.Text = String.Empty

            Dim MaxId As String = String.Empty
            Dim MaxSn As String = String.Empty
            Dim cpReader As SqlDataReader
            ExecuteSqlQuery(SqlDataAdapter, myDataSet, "Select Max(P_NUM) AS MAX_ID from PATIENT", "PATIENT", Connection)
            cpReader = SqlDataAdapter.SelectCommand.ExecuteReader
            While cpReader.Read
            ...
            ExecuteSqlQuery(EntryDataAdapter, SqlDataSet, "Select Max(P_SNO) AS MAX_SNO from PATIENT WHERE P_DATE_SAMPLE = CONVERT(DATETIME,'" & Format(DateTime.Parse(Date.Now.ToShortDateString), "MM/dd/yyyy") & " 00:00:00',102)", "PATIENT", Connection)
            ...
            cmbTitle.Focus()
            lblSequence.Text = Format(Now, "yyMMdd") + Format(Val(txtSNo.Text.Trim), "00") + txtRecNo.Text.Trim
        Catch EX As Exception
            MsgBox(EX.Message, MsgBoxStyle.Information, My.Application.Info.ProductName)
            Application.Exit()
        End Try
        ...
        Try
            ...
            If iCount > 0 And Sub_Addition = True Then
                Do Until iLoop = iCount
                    Dim str As Date = TestListView.Items(iLoop).SubItems(3).Text.ToString
                    CommandText = "INSERT INTO RECEIPT(TCASENO,TCODE,TNAME,TCATG,TRATE,TQTY,TDATE,TMODE,TDELETE,TRPTDATE) VALUES (" & _
                             ...
                             Format(DateTime.Parse(DateTime.Now.ToShortDateString.ToString), "MM/dd/yyyy") & "','ADD',0,'" & Format(DateTime.Parse(CDate(str)), "MM/dd/yyyy") & "')"
            ...
            Dim MaxSn As String = String.Empty
            Dim MaxSerialNo As SqlDataReader
            ExecuteSqlQuery(SqlDataAdapter, SqlDataSet, "Select Max(P_SNO) AS MAX_SNO from PATIENT WHERE P_DATE_SAMPLE = CONVERT(DATETIME,'" & Format(DateTime.Parse(Date.Now.ToShortDateString), "MM/dd/yyyy") & " 00:00:00',102)", "PATIENT", Connection)
            MaxSerialNo = SqlDataAdapter.SelectCommand.ExecuteReader
            While MaxSerialNo.Read
                MaxSn = MaxSerialNo.Item("MAX_SNO").ToString
            End While
            If Not MaxSerialNo Is Nothing Then MaxSerialNo.Close()
            txtRecNo.Text = GenerateMax_ID("patient", "p_num")
            txtSNo.Text = Val(MaxSn) + 1
            lblSequence.Text = Format(Now, "yyMMdd") + txtSNo.Text.Trim + txtRecNo.Text.Trim
            ...
            svDataRow.Item("P_SPC_REFER") = StrConv(txtSRefer.Text.Trim, VbStrConv.ProperCase)
            ...
            svDataRow.Item("P_REFER_2") = StrConv(txtHospital.Text.Trim, VbStrConv.Uppercase)
            ...
            svDataRow.Item("P_P_AMOUNT") = Val(txtPAmount.Text.Trim)
            svDataRow.Item("P_SEQUENCE") = lblSequence.Text.Trim
            ...
            cmdSave.Focus()
            Call Record_Count()
            MainReception.ToolStripStatusLabel8.Text = gblTotCases
            Me.ProcessTabKey(False)
        ...
        Try
            ...
            If objDataTable.Rows.Count > 0 Then
                For Each objDataRow In objDataTable.Rows
                    ...
                    txtPAmount.Text = objDataRow.Item("P_P_AMOUNT")
                    ...
                    If objDataRow.Item("P_REASON").ToString.Trim <> "X" Or objDataRow.Item("P_REASON").ToString.Trim <> "x" Then lblRemarks.Text = "Modified on " & _
                       objDataRow.Item("P_EDIT_ON") & " by " & objDataRow.Item("P_EDIT_BY").ToString.Trim & " :-" & Space(5) & objDataRow.Item("P_REASON").ToString.Trim Else lblRemarks.Text = ""
    ...
    Private Sub cmdLSlip_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdLSlip.Click
        ...
        If Len(gblFind) > 0 Then
            gblReportOption = "RECEIPT"
            gblReportName = ".\" & "Reports\LabSlip.rpt"
            'gblBarcodeRpt = ".\" & "Reports\BarcodeTag.rpt"
            Call Print_Slips_Manually()
            'Call Print_Barcode_Tag()
        End If
    End Sub
    ...
    Private Sub txtRecNo_MouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles txtRecNo.MouseDoubleClick
        If gblPrliviges = 1 Then
            StartID = Microsoft.VisualBasic.InputBox("Please give Start ID.", My.Application.Info.ProductName, vbOKCancel)
