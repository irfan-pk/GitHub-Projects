...
Public Class Payroll
    ...
    Dim EmployeeStatus As String = String.Empty
    Dim addMode As Boolean = False
    Dim editMode As Boolean = False
    Dim SheetExist As Boolean = False
    Dim DeleteId, EmployeeID As Integer
    ...
    Private Sub dgvEmpList_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles dgvEmpList.KeyDown
        If dgvEmpList.RowCount > 0 Then
            If e.KeyCode = Keys.Enter Then
                EmployeeID = Convert.ToInt32(dgvEmpList.Rows(dgvEmpList.CurrentRow.Index).Cells(2).Value.ToString)
            End If
        End If
    End Sub
    ...
    Private Sub cmdAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdAdd.Click
        If EmpTabControl.SelectedIndex = 1 Then
            If dgvEmpList.RowCount > 0 Then EmployeeID = Convert.ToInt32(dgvEmpList.Rows(dgvEmpList.CurrentRow.Index).Cells(2).Value.ToString)
            gblSalaryAdd = True
            gblSalaryEdit = False
    ...
    Private Sub dgvEmpList_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvEmpList.CellClick
        ...
        If EmpTabControl.SelectedIndex = 2 Then
            If EmployeeStatus <> "Active" Then
                MessageBox.Show("This Employee has been retired/terminate.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
        End If
    End Sub
    ...
    Private Sub EmpTabControl_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles EmpTabControl.SelectedIndexChanged
        If dgvEmpList.RowCount > 0 Then
            EmployeeID = Convert.ToInt32(dgvEmpList.Rows(dgvEmpList.CurrentRow.Index).Cells(2).Value.ToString)
            If EmpTabControl.SelectedIndex = 1 Then Call ShowRows() : dgvEmpList.Enabled = True
            If EmpTabControl.SelectedIndex = 2 Then Call ShowAdvances() : dgvEmpList.Enabled = True
            'If EmpTabControl.SelectedIndex = 3 Then dgvFinalSalary.Columns("colPayMonth").Visible = False : dgvEmpList.Enabled = False
        End If
    End Sub

    Public Sub ShowRows()
        SalaryDataSet.Clear()
        Dim ShowRows As String = "select * from salary_updation where EmployeeName = '" & gblEmpName & "' order by IncreaseOn desc"
        Execute_DatabaseTable_Query(SqlDataAdapter, ShowRows, Connection)
        SqlDataAdapter.Fill(SalaryDataSet, "salary_updation")
        SalaryBindingSource.DataSource = SalaryDataSet
        SalaryBindingSource.DataMember = "salary_updation"
        dgvSalary.Refresh()
        dgvEmpList.Refresh()
    End Sub
    ...
    Private Sub cmdDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDelete.Click
        If dgvSalary.RowCount > 0 Then
            DeleteId = Convert.ToInt32(dgvSalary.Rows(dgvSalary.CurrentRow.Index).Cells(5).Value.ToString)
            If MsgBox("Are you sure to delete the entry for " & EmployeeID & " ?", MsgBoxStyle.Information + MsgBoxStyle.OkCancel, "Delete Record") = MsgBoxResult.Ok Then
                Dim CommandText As String = "DELETE FROM SALARY_UPDATION WHERE INCREASEON = '" & Format(DateTime.Parse(dgvSalary.Rows(dgvSalary.CurrentRow.Index).Cells(0).Value.ToString), "MM/dd/yyyy") & "' AND UPDATESEQ = " & DeleteId
    ...
    Private Sub txtAdvance_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtAdvance.KeyPress
        If Not (Char.IsDigit(e.KeyChar) Or Char.IsControl(e.KeyChar) Or (e.KeyChar = "-") Or e.KeyChar.ToString.Equals(System.Globalization.CultureInfo.CurrentCulture.NumberFormat.NumberDecimalSeparator)) Then e.Handled = True
    End Sub

    Public Sub GetEmployeeInfo(ByVal EmpId As Integer)
                ...
                Using reader As SqlDataReader = sqlCmd.ExecuteReader
                    If reader.HasRows Then
                        While reader.Read
                            ...
                            EmployeeStatus = reader.Item("status").ToString
                            gblEmpName = reader.Item("firstname").ToString & Space(1) & reader.Item("lastname").ToString
                        End While
                    End If
                End Using
            End Using
    ...
    Private Sub ShowAdvances()
        ...
        Call GetEmployeeInfo(Convert.ToInt32(dgvEmpList.Rows(dgvEmpList.CurrentRow.Index).Cells(2).Value.ToString))
        txtID.Text = gblEmpID
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        If Len(txtAdvance.Text) = 0 Or Val(txtAdvance.Text) = 0 Then Exit Sub
        If Val(txtAdvance.Text.Trim) > 0 Or Val(txtAdvance.Text) < 0 Then
            Dim StringMonth As String = Format(dtpAdvDate.Value, "MM/yyyy")
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        If dgvAdvances.RowCount > 0 Then
            Dim DeleteID As Integer = Val(dgvAdvances.Rows(dgvAdvances.CurrentRow.Index).Cells(0).Value.ToString)
            If MsgBox("Are you sure to delete the entry of " & dgvAdvances.Rows(dgvAdvances.CurrentRow.Index).Cells(1).Value.ToString & " (" & dgvAdvances.Rows(dgvAdvances.CurrentRow.Index).Cells(3).Value.ToString & ") ?", MsgBoxStyle.Information + MsgBoxStyle.OkCancel, "Delete Record") = MsgBoxResult.Ok Then
                Dim CommandText As String = "DELETE FROM SALARY_ADVANCES WHERE EMPLOYEEID = " & DeleteID & " AND ADVDATE = '" & Format(DateTime.Parse(dgvAdvances.Rows(dgvAdvances.CurrentRow.Index).Cells(3).Value.ToString), "MM/dd/yyyy") & "'"
                Execute_DatabaseTable_Query(SqlDataAdapter, CommandText, Connection)
    ...
    Private Sub btnCreate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCreate.Click
        ...
        If SheetExist = False Then
            ...
            Dim CopyNewRows As String = "insert into salary_sheet(empid,empname,designation,empsalary,emparears,salarymonth,prepareby,status) " & _
                                        "select employeeid,firstname + ' ' + lastname,position,salary,arear,'" & Format(DateTime.Parse(dtpMonth.Value.ToShortDateString), "MMM - yyyy") & "','" & SysOperator.Trim & "','" & "OPEN" & "' from employees where status = 'Active'"
            Execute_DatabaseTable_Query(PaySqlAdapter, CopyNewRows, Connection)
            Call CalculateAdvances()
        End If
        Reader.Close()
        Call RefreshSheetGrid()
        Call HighlightGridCell()
    End Sub
    ...
    Private Sub UpdateGrossSalary()
        Dim connString As String = ("Data Source=" & ServerName & ";Initial Catalog=" & ServerDatabase & ";NETWORK LIBRARY=DBMSSOCN;User ID=" & ServerID & ";Password=" & ServerPwd & ";DATABASE=" & ServerDatabase & ";")
        Dim cmdText As String = _
           ("select employeeid, sum(salary) as gross_pay from employees group by EmployeeID")
        Dim bRet As Boolean = False
        Using sqlConnection As SqlConnection = New SqlConnection(connString)
            sqlConnection.Open()
            Using sqlCmd As SqlCommand = New SqlCommand(cmdText, sqlConnection)
                Using reader As SqlDataReader = sqlCmd.ExecuteReader
                    If reader.HasRows Then
                        While reader.Read
                            Call ProcessTransaction("update salary_sheet set empsalary = " & Val(reader.Item("gross_pay").ToString) & " where empid = " & Val(reader.Item("employeeid").ToString) & " and salarymonth = N'" & Format(dtpMonth.Value, "MMM - yyyy") & "'")
                        End While
                    End If
                    If reader Is Nothing Then reader.Close()
                End Using
    ...
    Private Sub btnFinalize_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFinalize.Click
        ...
        If dgvFinalSalary.RowCount > 0 Then
            If MsgBox("Are you sure to finalize the salary of month ( " & Format(DateTime.Parse(dtpMonth.Value.ToShortDateString), "MMM - yyyy") & " ) ?", MsgBoxStyle.Information + MsgBoxStyle.OkCancel, "Finalize Salary") = MsgBoxResult.Ok Then
    ...
    Private Sub btnReverse_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnReverse.Click
        If gblPrliviges > 1 Then
            ...
            Exit Sub
        End If
        Dim response As DialogResult = MessageBox.Show("Are you roll back salary sheet for the month of " & Format(dtpMonth.Value, "MMM - yyyy"), "Confirm", MessageBoxButtons.OKCancel, MessageBoxIcon.Question)
        If response = System.Windows.Forms.DialogResult.OK Then
            With dgvFinalSalary
                For Each row As DataGridViewRow In dgvFinalSalary.Rows
                    Dim UpdateArrear As String = "update employees set arear = " & Val(row.Cells(5).Value.ToString) & " where employeeid = " & Val(row.Cells(1).Value.ToString)
                    If Val(row.Cells(5).Value.ToString) > 0 And row.Cells(11).Value.ToString = "FINALIZED" Then
                        Call ProcessTransaction(UpdateArrear)
                    End If
                Next
    ...
    Private Sub HighlightGridCell()
        For Each gRow As DataGridViewRow In dgvFinalSalary.rows
            If Val(gRow.Cells(9).Value) > 0 Then
                gRow.Cells(9).Style.BackColor = Color.Green
                gRow.Cells(9).Style.ForeColor = Color.White
            Else
                gRow.Cells(9).Style.BackColor = Color.White
                gRow.Cells(9).Style.ForeColor = Color.Black
            End If
            If Val(gRow.Cells(10).Value) > 0 Then
                gRow.Cells(10).Style.BackColor = Color.Red
                gRow.Cells(10).Style.ForeColor = Color.White
            Else
                gRow.Cells(10).Style.BackColor = Color.White
                gRow.Cells(10).Style.ForeColor = Color.Black
            End If
        Next
    End Sub

    Private Sub BtnRefresh_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnRefresh.Click
        If dgvFinalSalary.Rows(dgvFinalSalary.CurrentRow.Index).Cells(11).Value.ToString = "FINALIZED" Then
            MessageBox.Show("Salary Sheet Finalized, Changes cannot be made.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        Else
            Call CalculateAdvances()
            Call UpdateGrossSalary()
            Call RefreshSheetGrid()
            Call HighlightGridCell()
        End If
    End Sub
    ...
    Private Sub btnDel_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDel.Click
        If dgvFinalSalary.Rows(dgvFinalSalary.CurrentRow.Index).Cells(11).Value.ToString = "FINALIZED" Then
            ...
            Exit Sub
        Else
            Dim response As DialogResult = MessageBox.Show("Are you sure to delete current sheet ? ", "Confirm Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
    ...
    Private Sub btnUpdateSalary_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpdateSalary.Click
        If dgvFinalSalary.Rows(dgvFinalSalary.CurrentRow.Index).Cells(11).Value.ToString = "FINALIZED" Then
            MessageBox.Show("Salary Sheet Finalized, Changes cannot be save.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If
        With dgvFinalSalary
            For k = 0 To .RowCount - 1
                Dim ChangeCommand As String = "update salary_sheet set empdeduction = " & Val(.Rows(k).Cells(8).Value.ToString) & " " & _
                                              "where empid = " & Val(.Rows(k).Cells(1).Value.ToString) & " and salarymonth = N'" & Format(dtpMonth.Value, "MMM - yyyy") & "'"
                Call ProcessTransaction(ChangeCommand)
            Next
        End With
    End Sub
End Class