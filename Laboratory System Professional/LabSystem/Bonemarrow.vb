...
Public Class Bonemarrow
    ...
    Private Sub txtPatientID_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtPatientID.KeyDown
        If e.KeyCode = Keys.Enter And txtPatientID.Text <> String.Empty Then
            If e.KeyCode = Keys.Enter And txtPatientID.Text <> String.Empty Then
                ...
                Check = "select * from lab_pro.dbo.receipt where tcaseno = " & Val(txtPatientID.Text.Trim) & " and (tname like '%MARROW%')"
    ...
    Private Sub Bonemarrow_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        txtPatientID.Focus()
    End Sub
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        ...
        Dim SaveData As String = "select * from lab_pro.dbo.rpt_biopsy where labno=" & Val(txtPatientID.Text.Trim) & " and rptcode = 'MARROW'"
    ...
    Private Sub cmdPrintReport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrintReport.Click
        ...
        gblPrintReportCode = "MARROW"
        gblPrintFileName = CurrentPath & "\Pathology\" & "BONE MARROW.rpt"
        gblRptRmkDetail = String.Empty
        gblRptRmkCode = String.Empty
    ...
    Private Sub Edit_Report_Data(ByVal TableName As String, ByVal PatiendId As String)
        Try
            Call GetConnectionString()
            CheckQuery = "Select * from lab_pro.dbo.rpt_biopsy where labno = " & Val(PatiendId.Trim) & " and rptcode = 'MARROW' order by labno"
            ...
            If GridData.HasRows Then
                While GridData.Read
                    txtBMarowNo.Text = GridData.Item("biopsy_id").ToString.Trim
    ...
    Private Function CheckReport(ByVal TableName As String) As Boolean
        ...
        Dim cmdText As String = _
           ("select * from " & TableName & " where labno=" & Val(txtPatientID.Text.Trim) & " and rptcode = 'MARROW'")
    ...
    Private Sub txtMicroscope_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtMicroscope.KeyPress
    ...
    Private Sub txtBMarowNo_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtBMarowNo.KeyPress
    ...
    Private Function CultureReportExist(ByVal PatientID As String, ByVal ReportCode As String) As Boolean
        Dim connString As String = ("Data Source=" & ServerName & ";Initial Catalog=" & ServerDatabase & ";NETWORK LIBRARY=DBMSSOCN;User ID=" & ServerID & ";Password=" & ServerPwd & ";DATABASE=" & ServerDatabase & ";")
        Dim cmdText As String = _
           ("select * from lab_pro.dbo.rpt_biopsy where labno=" & Val(PatientID.Trim) & " and rptcode = 'MARROW'")
    ...
    Private Sub txtConclusion_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtConclusion.KeyPress
        If Char.IsLetter(e.KeyChar) Or e.KeyChar = ControlChars.Back Or Char.IsWhiteSpace(e.KeyChar) Or e.KeyChar = "." Then
            e.Handled = False
        Else
            e.Handled = True
    ...
    Private Sub QueryWithParameter(ByVal QueryCommand As String, ByVal Mode As String)
        ...
        Using sqlConnection As SqlConnection = New SqlConnection(connString)
            ...
            Using sqlCmd As SqlCommand = New SqlCommand(cmdText, sqlConnection)
                ...
                sqlCmd.Parameters.Add(New SqlParameter("@BiopsyNo", txtBMarowNo.Text.Trim))
                sqlCmd.Parameters.Add(New SqlParameter("@ClinicalHist", "x"))
                ...
                sqlCmd.Parameters.Add(New SqlParameter("@RptCode", "MARROW"))
                sqlCmd.Parameters.Add(New SqlParameter("@FileName", "BONE MARROW.rpt"))
                ...
                If Mode = "UPDATE" Then
                    ...
                    sqlCmd.Parameters.Add(New SqlParameter("@UpdateRptCode", "MARROW"))
                End If
