...
Public Class UrineDr
    ...
    Private Sub Report_Data(ByVal TableName As String)
        Try
            Dim FetchReportData As String = "select * from " & TableName & " where TSTCODE = N'" & gblRptTstCode & "' ORDER BY SORT_ORDER"
    ...
    Private Sub ReportDataGrid_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles ReportDataGrid.CellEndEdit
        With ReportDataGrid
            If ReportDataGrid.Rows(.CurrentRow.Index).Cells(0).Value.ToString = "PATIENT" Then
                ...
                .Rows(.CurrentRow.Index + 3).Cells(1).Value = Format(Val(.Rows(.CurrentRow.Index + 2).Cells(1).Value.ToString) ^ 1.2, "0.00")
                .Rows(.CurrentRow.Index + 4).Cells(1).Value = Format(Val(.Rows(.CurrentRow.Index + 1).Cells(1).Value.ToString) / Val(.Rows(.CurrentRow.Index).Cells(1).Value.ToString) * 100, "0.00")
            End If
        End With
    ...
    Private Sub ReportDataGrid_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles ReportDataGrid.KeyPress
        If Char.IsLetterOrDigit(e.KeyChar) Or e.KeyChar = ControlChars.Back Or e.KeyChar = "." Or e.KeyChar = "-" Or e.KeyChar = "+" Or Char.IsWhiteSpace(e.KeyChar) Then
            e.Handled = False
        Else
            e.Handled = True
        End If
    ...
    Private Sub UrineDr_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
    ...
    Private Sub ReportDataGrid_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles ReportDataGrid.KeyUp
        ...
        If e.KeyCode = Keys.F4 Then
            MultipleStone.ShowDialog(Me)
            If gblStoneSize = String.Empty Then Exit Sub
            ReportDataGrid.Rows(0).Cells(1).Value = gblStoneSize
        End If
    End Sub
