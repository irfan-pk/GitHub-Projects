...
Public Class WholeSalerLedger
    ...
    Private Sub WholeSalerLedger_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Call Popup_Data("SELECT * FROM SUPPLIERS_JOURNAL WHERE SUPL_TR_DT = N'" & Format(Date.Parse(TranDate.Value.ToString), "MM/dd/yyyy") & "' ORDER BY SUPL_TR_ID")
        Connection.Close()
    End Sub
    ...
    Private Sub txtID_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtID.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            If GetName.HasRows Then
                ...
                Call Popup_Data("SELECT * FROM SUPPLIERS_JOURNAL WHERE SUPL_CODE = N'" & Val(gblCP_ID) & "' ORDER BY SUPL_TR_ID")
                txtVoucher.Focus()
            Else
    ...
    Private Sub Popup_Data(ByVal StrQuery As String)
        Try
            Max_Transaction = GenerateMax_ID("SUPPLIERS_JOURNAL", "SUPL_TR_ID")
            txtTran.Text = Max_Transaction.ToString.Trim
            Me.PaymentDataSet.Clear()
            Dim CommandText As String = StrQuery
            ExecuteAdapterQuery(SqlDataAdapter, CommandText, Connection)
            SqlDataAdapter.Fill(Me.PaymentDataSet, "SUPPLIERS_JOURNAL")
    ...
    Private Sub Updt_General_Journal(ByVal PayType As String)
        ...
        If PayType.ToUpper.Trim = "DEBIT" Then
            ...
            cmdText = ("INSERT INTO SUPPLIERS_JOURNAL VALUES ('" & Supplier_Code & "','" & Format(Date.Parse(TranDate.Value.ToString), "MM/dd/yyyy") & "','" & Val(txtTran.Text.Trim) & "','" & txtName.Text.Trim & "','" & "Doc # " & txtVoucher.Text.ToUpper & Space(1) & txtDescription.Text.Trim.ToUpper & "'," & Opn_Bal & "," & Val(txtDebit.Text.Trim) & ",0," & Clo_Bal & ",'" & Format(Date.Parse(TranDate.Value.ToString), "MM/dd/yyyy") & "')")
        ...
        Call Popup_Data("SELECT * FROM SUPPLIERS_JOURNAL WHERE SUPL_CODE = N'" & Val(txtID.Text.Trim) & "' ORDER BY SUPL_TR_ID")
        txtCredit.Text = 0
        txtDebit.Text = 0
        txtVoucher.Clear()
        txtDescription.Clear()
