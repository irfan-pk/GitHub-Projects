...
Public Class PurchaseInvoiceCorrection
    ...
    Dim Items, Clos_Balance, LastPrchID, HoldPurchInvoice, PurchaseType, PurchaseStatus, UpdateAccount As String
    Public Payment, FinalPayment, TotalQty, Dues, ProdPrice, Discount, NetTotal, NetAmount, wGross, wDiscount, wTotal, wPaid, wCredit As Double
    ...
    Private Sub SearchInvoiceHelp()
        gblFind = InputBox("Please Enter Invoice # to Search.", "Search Invoice", "0")
        If Not IsNumeric(gblFind) Then MsgBox("Invalid Invoice No.", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Invlaid") : Exit Sub
        RefreshFields()
        ShowPurchaseInvoice()
    End Sub

    Private Sub ShowPurchaseInvoice()
        'Invoice Basic
        ExecuteAdapterQuery(SqlDataAdapter, "select * from purchase_master where prchid = " & Val(gblFind), Connection)
        Dim Reader As SqlDataReader = SqlDataAdapter.SelectCommand.ExecuteReader
        If Reader.HasRows Then
            While Reader.Read
                'Code Here
                txtSuplCode.Text = Reader.Item("supplrid").ToString()
                lblSuplrName.Text = Reader.Item("supplrname").ToString()
                lblDarawer.Text = Reader.Item("drawername").ToString()
                lblInvoice.Text = Reader.Item("prchid").ToString()
                lblInvoiceDate.Text = Reader.Item("datecompleted").ToString()
                lblInvoiceTotal.Text = Reader.Item("prchgross").ToString()
                wGross = Val(Reader.Item("prchgross").ToString())
                wDiscount = Val(Reader.Item("prchdiscount").ToString())
                wTotal = Val(Reader.Item("prchtotal").ToString())
                wPaid = Val(Reader.Item("prchpaid").ToString())
                wCredit = Val(Reader.Item("prchcredit").ToString())
                txtDiscount.Text = wDiscount
                lblNetTotal.Text = wCredit
                txtCashPaid.Text = wPaid
                wSuplCode = Val(txtSuplCode.Text)
            End While
        Else
            MsgBox("Invalid Invoice, Please give valid invoice no", MsgBoxStyle.OkOnly + MsgBoxStyle.Information)
            Exit Sub
        End If
        If Reader Is Nothing Then Reader.Close()
        'Invoice Details
        ExecuteAdapterQuery(SearchAdapter, "select * from purchase_detail where prchid = " & Val(gblFind), Connection)
    ...
    Private Sub SaveInvoice()
        'Code Here
        Dim invoice As Integer = Val(lblInvoice.Text)
        Dim suplcode As Integer = Val(txtSuplCode.Text)
        Dim total As Double = Val(lblInvoiceTotal.Text)
        Dim discount As Double = Val(txtDiscount.Text)
        Dim cash As Double = Val(txtCashPaid.Text)
        Dim pdate As Date = Date.Now
        Dim soperator As String = SysOperator.ToUpper
        Dim supplier As String = lblSuplrName.Text.Trim

        Dim OrderStatus As String = String.Empty
        generateInvoice = False
        'Check whether invoice debit or credit
        If Val(lblNetTotal.Text) = Val(lblInvoiceTotal.Text) - gblDiscount Then OrderStatus = "CREDIT" : generateInvoice = True
        If Val(txtCashPaid.Text) = Val(lblInvoiceTotal.Text) - gblDiscount Then OrderStatus = "CASH" : generateInvoice = True
        If generateInvoice = True Then
            If SaleDataGrid.Rows.Count > -1 Then
                'For Deletion old stock
                UpdateReturnStockQuantity(invoice)
                ProcessTransaction("delete from purchase_detail where prchid = " & invoice)
                ...
                If SaleDataGrid.RowCount > -1 Then
                    Dim myQuery As String = "UPDATE purchase_master SET " & _
                                                         "PRCHID = " & invoice & "," & _
                                                         "DRAWERID = " & DrawerID & "," & _
                                                         "DRAWERNAME = '" & soperator & " '," & _
                                                         "DATECOMPLETED = '" & Format(Date.Parse(pdate), "MM/dd/yyyy") & "'," & _
                                                         "PRCHTYPE = '" & OrderStatus & "'," & _
                                                         "OFFLINEID = '" & DetailID.Trim & " '," & _
                                                         "SUPPLRID = " & suplcode & "," & _
                                                         "SUPPLRNAME = '" & supplier & "'," & _
                                                         "PRCHGROSS = " & total & "," & _
                                                         "PRCHDISCOUNT = " & discount & "," & _
                                                         "PRCHTOTAL = " & gblNet - gblDiscount & "," & _
                                                         "PRCHPAID = " & cash & "," & _
                                                         "LEDGERPOST = 0" & "," & _
                                                         "STATUS = '" & OrderStatus & "')"
                    ProcessTransaction(myQuery)
                    SortID = 1
                    For Each OutRow As DataGridViewRow In SaleDataGrid.Rows
                        Dim SaveItemQuery As String = "INSERT INTO purchase_detail VALUES ('" & _
                                                             DetailID.Trim & "','" & _
                                                             Format(Date.Parse(pdate), "MM/dd/yyyy") & "'," & _
                                                             invoice & ",'" & _
                                                             OrderStatus.ToUpper & "'," & _
                                                             suplcode & ",'" & _
                                                             supplier & "'," & _
                ...
                UpdateStockQuantity(invoice)
                SortID = 0
                If wGross > Val(lblInvoiceTotal.Text) Then LedgerDrTransaction(2, "PO-" & lblInvoice.Text, "CORRECTION IN PURCHASE ON", "DR", wGross - Val(lblInvoiceTotal.Text.Trim))
                If wDiscount > Val(gblDiscount) Then LedgerCrTransaction(3, "PO-" & lblInvoice.Text, "CORRECTION IN PURCHASE DISCOUNT", "CR", wDiscount - gblDiscount)
                If wPaid > gblPayment And wSuplCode = Val(txtSuplCode.Text) Then LedgerCrTransaction(Val(txtSuplCode.Text), "PO-" & lblInvoice.Text, "CORRECTION IN PURCHASE PAYMENT", "CR", wPaid - gblPayment)

                If wGross < Val(lblInvoiceTotal.Text) Then LedgerCrTransaction(2, "PO-" & lblInvoice.Text, "CORRECTION IN PURCHASE ON", "CR", Val(lblInvoiceTotal.Text.Trim) - wGross)
                If wDiscount < Val(gblDiscount) Then LedgerDrTransaction(3, "PO-" & lblInvoice.Text, "CORRECTION IN PURCHASE DISCOUNT", "DR", gblDiscount - wDiscount)
                If wPaid < gblPayment Then LedgerDrTransaction(Val(txtSuplCode.Text), "PO-" & lblInvoice.Text, "CORRECTION IN PURCHASE PAYMENT", "DR", gblPayment - wPaid)
            ...
            End If
        Else
            MessageBox.Show("Partial payment not allowed at the time purchase order.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Question)
        End If
    End Sub
    ...
    Private Sub txtSuplCode_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtSuplCode.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            End If
            ExecuteAdapterQuery(SqlDataAdapter, "select * from accounts where acc_no = " & Val(txtSuplCode.Text) & " and acc_reg_type = 'COMPANY'", Connection)
            Dim reader As SqlDataReader = SqlDataAdapter.SelectCommand.ExecuteReader
            If reader.HasRows Then
                While reader.Read
                ...
                Panel2.Enabled = True
            Else
                MsgBox("Please select/give valid supplier code", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Invalid")
                Exit Sub
            End If
            If reader Is Nothing Then reader.Close()
            ExecuteAdapterQuery(InfoDataAdapter, "select max(prchid) as lastInvoice from purchase_master where supplrid = " & Val(txtSuplCode.Text), Connection)
    ...
    Private Sub UpdateReturnStockQuantity(ByVal InvoiceNo As Integer)
    ...
    Private Sub UpdateStockQuantity(ByVal InvoiceNo As Integer)
        Dim SelectQty As String = "SELECT * FROM PURCHASE_DETAIL WHERE PrchID = " & InvoiceNo
        ExecuteAdapterQuery(EntryDataAdpter, SelectQty, Connection)
        Dim Updater As SqlDataReader = EntryDataAdpter.SelectCommand.ExecuteReader
        If Updater.HasRows Then
    ...
    Private Sub txtDiscount_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtDiscount.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            lblNetTotal.Text = Format(Convert.ToDouble(lblInvoiceTotal.Text) - gblDiscount, "0.00")
            txtCashPaid.Focus()
        End If
    End Sub
    ...
    Private Sub txtCashPaid_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtCashPaid.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            If Val(txtCashPaid.Text) > 0 Then
                lblNetTotal.Text = Format(Convert.ToDouble(lblNetTotal.Text) - Convert.ToDouble(txtCashPaid.Text), "0.00")
    ...
    Private Sub SaleDataGrid_KeyUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles SaleDataGrid.KeyUp
