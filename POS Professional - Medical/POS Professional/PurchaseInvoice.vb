...
Public Class PurchaseInvoice
    ...
    Private Sub PurchaseInvoice_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
        ...
        If e.KeyCode = Keys.F5 Then If lblNet.Enabled = True Then txtDiscount.Focus()
        If e.KeyCode = Keys.F11 Then RefreshFields()
        If e.KeyCode = Keys.PageUp AndAlso e.Modifiers = Keys.Control Then If lblDelete.Enabled = True Then DeleteItem()
    ...
    Private Sub SaveInvoice()
        ...
        Dim credit As Double = Val(lblNetTotal.Text)
        ...
        If generateInvoice = True Then
            If SaleDataGrid.Rows.Count > -1 Then
                ...
                If SaleDataGrid.RowCount > -1 Then
                    Dim MasterQuery As String = "INSERT INTO purchase_master VALUES (" & _
                                                         invoice & "," & _
                                                         DrawerID & ",'" & _
                                                         soperator & "','" & _
                                                         ...
                                                         total & "," & _
                                                         discount & "," & _
                                                         gblNet - gblDiscount & "," & _
                                                         cash & "," & _
                                                         gblNet - cash - gblDiscount & "," & _
                                                         "0" & ",'" & _
                                                         Format(Date.Parse(pdate), "MM/dd/yyyy") & "'," & _
                                                         "0" & ",'" & _
                                                         OrderStatus & "')"
                ...
                SortID = 0
                If OrderStatus = "CASH" Then
                    LedgerDrTransaction(4, "PO-" & invoice, "CASH PURCHASES", "DR", total)
                    If gblDiscount > 0 Then LedgerCrTransaction(5, "PO-" & invoice, "DISCOUNT ON PURCHASES", "CR", gblDiscount)
                    LedgerCrTransaction(1, "PO-" & invoice, "CASH PURCHASES", "CR", cash)
                    ProcessTransaction("update purchase_master set ledgerpost = 1 where prchid = " & invoice)
                End If
                If OrderStatus = "CREDIT" Then
                    LedgerDrTransaction(4, "PO-" & invoice, "CREDIT PURCHASES", "DR", total)
                    If gblDiscount > 0 Then LedgerCrTransaction(5, "PO-" & invoice, "DISCOUNT ON PURCHASES", "CR", gblDiscount)
                    LedgerCrTransaction(suplcode, "PO-" & invoice, "CREDIT PURCHASES", "CR", gblNet - cash - gblDiscount)
                    ProcessTransaction("update purchase_master set ledgerpost = 1 where prchid = " & invoice)
                End If
                UpdateStockQuantity()
                PrchReturn = True
