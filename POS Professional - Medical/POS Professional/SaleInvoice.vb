...
Public Class SaleInvoice
    ...
    Public Payment, FinalPayment, TotalQty, Dues, ProdPrice, Discount, NetTotal, NetAmount, SaleChange, OldBalance As Double
    ...
    Private Sub SaleInvoice_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        txtCustID.Focus()
    End Sub
    ...
    Private Sub SearchInvoiceHelp()
        gblQuery = "select saleid, salepaid from sale_master where drawerid = " & DrawerID & " and status = 'BILLED' order by saleid"
        SearchInvoice.ShowDialog(Me)
        If Len(gblCP_ID) > 0 Then ShowSaleInvoice()
    End Sub

    Private Sub ShowSaleInvoice()
        'Invoice Basic
        Totitems = 0
        If lblInvoice.Text = gblCP_ID Then MessageBox.Show("Invoice already searched and loaded.", "Exist", MessageBoxButtons.OK, MessageBoxIcon.Information) : Exit Sub
        ...
        If Reader.HasRows Then
            While Reader.Read
                ...
                gblDiscount = Val(Reader.Item("salediscount").ToString())
                gblNet = Val(Reader.Item("saletotal").ToString())
            End While
    ...
    Private Sub SaveInvoice()
        ...
        If gblDues = 0 And gblPayment > 0 Then BillMode = "BILLED" : generateInvoice = True
        If gblDues > 0 And Val(txtCustID.Text) = 1 Then MsgBox("Credit Invoice cannot be save to cash account.", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, "Message") : Exit Sub
        ...
        If generateInvoice = True Then
            If SaleDataGrid.Rows.Count > -1 Then
                ...
                SaveQuery = "INSERT INTO SALE_MASTER VALUES ('" & _
                                                     ...
                                                     Format(Val(lblInvoiceTotal.Text.Trim) + gblDiscount, "0.00") & "," & _
                                                     gblDiscount & "," & _
                                                     gblNet & "," & _
                                                     gblPayment & "," & _
                                                     "0" & ",'" & _
                ...
                'Ledger Entries
                If gblDues > 0 Then
                    If gblPayment > 0 Then LedgerDrTransaction(1, "SI-" & lblInvoice.Text, "PARTIAL PAYMENT ON " & SaleType, "DR", Val(gblPayment))
                    LedgerDrTransaction(Val(txtCustID.Text), "SI-" & lblInvoice.Text, "CREDIT SALE ( Acc # " & txtCustID.Text & " - " & lblCustomer.Text & ")", "DR", (gblDues))
                    If Val(gblDiscount) > 0 Then LedgerDrTransaction(3, "SI-" & lblInvoice.Text, "DISCOUNT ON SALE", "DR", Val(gblDiscount))
                    LedgerCrTransaction(2, "SI-" & lblInvoice.Text, "CREDIT SALE ( Acc # " & txtCustID.Text & " - " & lblCustomer.Text & ")", "CR", Val(lblInvoiceTotal.Text) + gblDiscount)
                    UpdateStockQuantity()
                End If
            ...
            End If
        Else
            MessageBox.Show("Invoice cannot be generate and print.", "Invalid", MessageBoxButtons.OK, MessageBoxIcon.Information)
            Exit Sub
        End If
        generateInvoice = False
    End Sub
    ...
    Private Sub txtRefer_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtRefer.KeyDown
        If e.KeyCode = Keys.Enter Then
            If Len(txtRefer.Text) > 0 Then
                ...
                End Try
            End If
            If Len(txtRefer.Text) = 0 Then lblRefer.Text = "Self"
            Panel2.Enabled = True
            txtCode.Focus()
        End If
    End Sub
    ...
    Private Sub txtRefer_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtRefer.LostFocus
        ...
        'If Len(lblRefer.Text) = 0 Then lblRefer.Text = "Self"
        'Panel2.Enabled = True
    End Sub
    ...
    Private Sub txtRefer_KeyUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtRefer.KeyUp
