...
Public Class frmSale
    ...
    Private Sub frmSale_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
        ...
        If e.KeyCode = Keys.F4 Then cmdCreditSale_Click(sender, e)
...
#Region "Runtime Functons and Procedures for Sale Form" '/CLICK ON EXPAND TO HIDE OR UNHIDE

    Private Sub cmdCreditSale_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCreditSale.Click
        If Val(lblOrgPrice.Text) - gblDiscount = Val(txtPaid.Text.Trim) - gblDiscount Then gblMessage = "Sale cannot be recorded as Credit Sale." : frmMsgStatus.ShowDialog(Me) : Exit Sub
        Dim difference As Integer = Val(lblOrgPrice.Text) - Val(txtPaid.Text) - gblDiscount
        If difference > 0 Then
            If SaleMode = True Then
                If lblCustName.Text.Trim = "" Or Len(lblCustName.Text.Trim) = 0 Then
                    gblMessage = "Credit not allowed to unknown customer."
                    frmMsgStatus.ShowDialog(Me)
                    Exit Sub
                End If
                ...
                Dim CustomerQuery As String = "UPDATE SALE_DETAIL_TEMP SET CustomerID = " & Val(lblCustID.Text.Trim) & ", CustomerName = N'" & lblCustName.Text.Trim & "' WHERE SaleID = N'" & Val(lblInvoice.Text.Trim) & "' and DetailID = N'" & Val(DetailID) & "'"
                ExecuteAdapterQuery(SaleDataAdapter, CustomerQuery, Connection)
            ...
            End If
        Else
            gblMessage = "Sale cannot be recorded as Credit Sale." : frmMsgStatus.ShowDialog(Me)
        End If
        txtCode.Focus()
    End Sub
                ...
                While GetStockInfo.Read
                    If CheckItemExist("SALE_DETAIL_TEMP") = False Then
                        TempDetailSale = "INSERT INTO SALE_DETAIL_TEMP VALUES (" & _
                                                 ...
                                                 GetStockInfo.Item("PROD_PACK").ToString.Trim & "'," & _
            ...
            Using sqlCmd As SqlCommand = New SqlCommand(cmdText, sqlConnection)
                Using reader As SqlDataReader = sqlCmd.ExecuteReader
                    While reader.Read
                        ...
                        lblTax.Text = Val(lblOrgPrice.Text.Trim) * 0.07
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        Payment = Val(lblOrgPrice.Text.Trim) - gblDiscount
        If Val(txtPaid.Text.Trim) < Payment Then gblMessage = "Payment Less than Total Amount." : frmMsgStatus.ShowDialog(Me) : txtPaid.Focus() : Exit Sub
        Dim SaveQuery As String = ""
