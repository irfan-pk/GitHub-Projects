...
Public Class ViewPurchases
    ...
    Private Sub cmdPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPreview.Click
        ...
        Else
            PurchaseQuery = "SELECT * FROM PURCHASE WHERE PRCH_DATE >= CONVERT(DATETIME,'" & Format(DateTime.Parse(FromDateTime.Text.Trim.ToString), "yyyy-MM-dd") & " 00:00:00',102) AND " & _
                                      "PRCH_DATE <= CONVERT(DATETIME,'" & Format(DateTime.Parse(ToDateTime.Text.Trim.ToString), "yyyy-MM-dd") & " 00:00:00',102) AND PRCH_CD = N'" & BindingNavigatorTextBox.Text.Trim & "' " & _
                                      "ORDER BY PRCH_TR_ID"
        End If
        Call View_Data(PurchaseQuery)
    End Sub
    ...
    Private Sub BindingNavigatorFind_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BindingNavigatorFind.Click
        If BindingNavigatorTextBox.Text = String.Empty Then MsgBox("Please select valid product code.", MsgBoxStyle.Information + MsgBoxStyle.OkOnly, My.Application.Info.ProductName) : Exit Sub
        PurchaseQuery = "SELECT * FROM PURCHASE WHERE PRCH_DATE >= CONVERT(DATETIME,'" & Format(DateTime.Parse(FromDateTime.Text.Trim.ToString), "yyyy-MM-dd") & " 00:00:00',102) AND " & _
                                      "PRCH_DATE <= CONVERT(DATETIME,'" & Format(DateTime.Parse(ToDateTime.Text.Trim.ToString), "yyyy-MM-dd") & " 00:00:00',102) AND PRCH_CD = N'" & BindingNavigatorTextBox.Text.Trim & "' " & _
                                      "ORDER BY PRCH_TR_ID"
        Call View_Data(PurchaseQuery)
