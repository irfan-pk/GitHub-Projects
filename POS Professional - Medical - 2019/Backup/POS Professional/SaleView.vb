...
Public Class SaleView
    ...
    Private Sub Popup_SaleData(ByVal TabChoice As String, ByVal TabDate As DateTimePicker)
        ...
        If TabChoice = "CashSale" Then MasterQuery = "SELECT SaleID, SaleType, CustomerID, CustomerName, SaleGross, SaleDiscount, SaleTotal, SalePaid, SaleCredit, SaleRecovery, SaleReturn, Status, DrawerName  FROM SALE_MASTER WHERE SaleCredit = 0 AND SaleReturn = 0 AND DateCompleted = N'" & Format(Date.Parse(TabDate.Value.ToString), "MM/dd/yyyy") & "' ORDER BY LEFT(SALEID,3), CONVERT(INT, RIGHT(SALEID, LEN(SALEID)-3))"
        If TabChoice = "CreditSale" Then MasterQuery = "SELECT SaleID, SaleType, CustomerID, CustomerName, SaleGross, SaleDiscount, SaleTotal, SalePaid, SaleCredit, SaleRecovery, SaleReturn, Status, DrawerName FROM SALE_MASTER WHERE SaleCredit > 0 AND DateCompleted = N'" & Format(Date.Parse(TabDate.Value.ToString), "MM/dd/yyyy") & "' ORDER BY LEFT(SALEID,3), CONVERT(INT, RIGHT(SALEID, LEN(SALEID)-3))"
        If TabChoice = "SaleRecovery" Then MasterQuery = "SELECT SaleID, SaleType, CustomerID, CustomerName, SaleGross, SaleDiscount, SaleTotal, SalePaid, SaleCredit, SaleRecovery, SaleReturn, Status, DrawerName FROM SALE_MASTER WHERE SaleRecovery > 0 AND DateRecovery = N'" & Format(Date.Parse(TabDate.Value.ToString), "MM/dd/yyyy") & "' ORDER BY LEFT(SALEID,3), CONVERT(INT, RIGHT(SALEID, LEN(SALEID)-3))"
        If TabChoice = "ReturnSale" Then MasterQuery = "SELECT SaleID, SaleType, CustomerID, CustomerName, SaleGross, SaleDiscount, SaleTotal, SalePaid, SaleCredit, SaleRecovery, SaleReturn, Status, DrawerName FROM SALE_MASTER WHERE SaleStatus = 'RETURN' AND DateCompleted = N'" & Format(Date.Parse(TabDate.Value.ToString), "MM/dd/yyyy") & "' ORDER BY LEFT(SALEID,3), CONVERT(INT, RIGHT(SALEID, LEN(SALEID)-3))"
