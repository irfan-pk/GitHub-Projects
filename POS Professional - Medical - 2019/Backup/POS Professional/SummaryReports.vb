...
Public Class SummaryReports
    ...
    Dim ReportAdapter As New SqlDataAdapter
    ...
    Private Sub frmSummaryReports_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            If gblReportFilter = "CASH" Then _
                CommandText = "Select * from SALE_MASTER WHERE DateCompleted >= CONVERT(DATETIME,'" & Format(FromDate, "yyyy-MM-dd") & " 00:00:00',102) AND DateCompleted <= CONVERT(DATETIME,'" & Format(ToDate, "yyyy-MM-dd") & " 00:00:00',102) AND (STATUS = 'PAID' OR STATUS = 'PARTIAL')" 'AND (SaleTotal > 0) AND (SalePaid > 0) OR (SaleReturn > 0)"

            If gblReportFilter = "CREDIT" Then _
                CommandText = "Select * from SALE_MASTER WHERE DateCompleted >= CONVERT(DATETIME,'" & Format(FromDate, "yyyy-MM-dd") & " 00:00:00',102) AND DateCompleted <= CONVERT(DATETIME,'" & Format(ToDate, "yyyy-MM-dd") & " 00:00:00',102) AND (SaleGross > 0) AND (SaleCredit > 0)"

            If gblReportFilter = "RECOVERY" Then _
                CommandText = "Select * from SALE_MASTER WHERE DateRecovery >= CONVERT(DATETIME,'" & Format(FromDate, "yyyy-MM-dd") & " 00:00:00',102) AND DateRecovery <= CONVERT(DATETIME,'" & Format(ToDate, "yyyy-MM-dd") & " 00:00:00',102) AND (SaleTotal > 0) AND (SaleRecovery > 0) AND (STATUS = 'PAID' OR STATUS = 'PARTIAL')"
            ...
            If gblReportFilter = "CRLIST" Then _
                CommandText = "Select * from SALE_MASTER WHERE DateCompleted >= CONVERT(DATETIME,'" & Format(FromDate, "yyyy-MM-dd") & " 00:00:00',102) AND DateCompleted <= CONVERT(DATETIME,'" & Format(ToDate, "yyyy-MM-dd") & " 00:00:00',102) AND (SaleGross > 0) AND (SaleCredit > 0)"

            ExecuteDatasetQuery(ReportAdapter, ReportDataSet, CommandText, "SALE_MASTER", Connection)
            ...
            objReport.SetDataSource(ReportDataSet.Tables("SALE_MASTER"))

            With objReport
                If gblReportOption = "STATEMENT" And gblReportFilter <> "CRLIST" Then
