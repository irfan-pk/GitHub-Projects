...
Public Class AccountRegistration
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If txtDrCr.Text = "DR" Then _
            OpeningBal = Convert.ToDecimal(Val(txtOpnDrBal.Text.Trim)) - Convert.ToDecimal(Val(txtCrLimit.Text.Trim))
        If txtDrCr.Text = "CR" Then _
            OpeningBal = Convert.ToDecimal(Val(txtCrLimit.Text.Trim)) - Convert.ToDecimal(Val(txtOpnDrBal.Text.Trim))
        ...
        If Editing = True And Addition = False Then Call ProcessTransaction("UPDATE ACCOUNTS SET ACC_NAME = '" & StrConv(txtAccName.Text.Trim, VbStrConv.ProperCase) & "'," & _
                                                           ...
                                                           " ACC_HEAD = '" & cmbAccHead.Text.ToUpper.Trim & "' WHERE ACC_NO = '" & EditString & "' AND ACC_NO > 7")

        '// if entered new values in text boxes then execute add record query
        If Addition = True And Editing = False Then Call ProcessTransaction("INSERT INTO ACCOUNTS VALUES('" & NewAccID & "','" & _
                                                           cmbAccHead.Text.ToUpper.Trim & "','" & _
                                                           cbAccReg.Text.ToUpper.Trim & "','" & _
                                                           txtAccName.Text.ToUpper.Trim & "','" & _
                                                           txtDrCr.Text.ToUpper.Trim & "'," & _
                                                           Convert.ToDecimal(Val(txtOpnDrBal.Text.Trim)) & "," & _
                                                           Convert.ToDecimal(Val(txtCrLimit.Text.Trim)) & "," & _
    ...
    Private Sub txtOpnCrBal_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles txtOpnCrBal.KeyPress
        ...
        Else
            ...
            If strCurrency.Length = 0 Then
                txtCrLimit.Text = ""
            ElseIf strCurrency.Length = 1 Then
                txtCrLimit.Text = "0.0" & strCurrency
            ElseIf strCurrency.Length = 2 Then
                txtCrLimit.Text = "0." & strCurrency
            ElseIf strCurrency.Length > 2 Then
                txtCrLimit.Text = strCurrency.Substring(0, strCurrency.Length - 2) & "." & strCurrency.Substring(strCurrency.Length - 2)
            End If
            txtCrLimit.Select(txtCrLimit.Text.Length, 0)
        End If
        e.Handled = True
    End Sub
    ...
    Private Sub txtAccID_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtAccID.KeyDown
        If e.KeyCode = Keys.Enter Then
            If Len(txtAccID.Text) > 0 Then
                ExecuteAdapterQuery(SelectAccAdapter, "select * from accounts order by acc_no", Connection)
                ...
                If SelectReader.HasRows Then
                    While SelectReader.Read
                        ...
                        txtBankAcc.Text = SelectReader.Item("acc_bankname").ToString()
