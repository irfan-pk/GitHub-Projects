...
Public Class SaleScreen
    ...
    Private Sub SaleScreen_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyUp
        ...
        If e.KeyCode = Keys.F7 Then cmdCreditSale_Click(sender, e)
        If e.KeyCode = Keys.F6 Then cmdSaleReturn_Click(sender, e)
    ...
    Private Sub lblQty_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        txtQty.Visible = True
        txtQty.Focus()
        txtQty.SelectAll()
    End Sub
                ...
                If SaleDataGrid.RowCount > -1 Then
                    For Each row As DataGridViewRow In SaleDataGrid.Rows
                        If row.Cells(0).Value.ToString.ToUpper = txtCode.Text.Trim Then
                            ...
                            SaleDataGrid.Rows(row.Index).Cells(4).Value = Val(SaleDataGrid.Rows(row.Index).Cells(4).Value.ToString) + Val(txtQty.Text.Trim)
                ...
                If GetStockInfo.HasRows Then
                    While GetStockInfo.Read
                        ...
                        If FoundItem = False Then
                            ...
                            'ItemRow.Cells(9).Value = GetStockInfo.Item("prod_sub_catg").ToString.ToUpper
                            SaleMode = True
                        End If
                        lblMode.Text = "Sale"
                        Call Get_Total()
    ...
    Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click
        If LastSaleID <> String.Empty Then
            ...
            txtCode.Focus()
        Else
            MessageBox.Show("Please record sale.", "Sale Receipt", MessageBoxButtons.OK, MessageBoxIcon.Information)
        End If
    End Sub
...
#Region "Runtime Functons and Procedures for Sale Form" '/CLICK TO HIDE OR UNHIDE
    ...
    Private Sub cmdCreditSale_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdCreditSale.Click
        If SaleMode = True Then
            If Val(lblCustID.Text) < 1 Then MessageBox.Show("Credit Customer Not Selected.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information) : Exit Sub
            Dim Data As String = String.Empty
            ...
            If SaleMode = True Then
                ...
                MasterQuery = "INSERT INTO SALE_MASTER VALUES ('" & _
                            ...
                            gblPayment & "," & _
                            "0" & "," & _
                            "'ACCOUNT'," & Val(lblNetDues.Text.Trim) & ",0,0,'" & Format(Date.Parse(lblDate.Text.Trim), "MM/dd/yyyy") & "',0,'BILLED','" & txtReference.Text & "')"
                ...
                If SaleDataGrid.RowCount > -1 Then
                    ...
                    For Each OutRow As DataGridViewRow In SaleDataGrid.Rows
                        HoldQuery = "INSERT INTO SALE_DETAIL VALUES ('" & _
                                                             ...
                                                             lblUserID.Text.Trim & "',0,0,'" & OutRow.Cells(7).Value.ToString & "','" & OutRow.Cells(8).Value.ToString & "',0,'x'," & SortID & ",'" & OutRow.Cells(9).Value.ToString & "'," & Val(OutRow.Cells(5).Value.ToString) & ",'" & txtReference.Text & "')"
                ...
                Call LedgerDrTransaction(Val(lblCustID.Text), "SI-" & lblInvoice.Text, "CREDIT SALE", "DR", Val(lblNetDues.Text))
                If Val(gblDiscount) > 0 Then Call LedgerDrTransaction(3, "SI-" & lblInvoice.Text, "SALE DISCOUNT", "DR", Val(gblDiscount))
                Call LedgerCrTransaction(2, "SI-" & lblInvoice.Text, "CREDIT SALE ( Acc #" & lblCustID.Text & " - " & lblCustName.Text & ")", "CR", Val(lblOrgPrice.Text))
    ...
    Private Sub txtCode_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtCode.KeyDown
        If e.KeyCode = Keys.Enter Then
            If txtCode.Text = String.Empty Or Len(txtCode.Text) < 2 Then
                MsgBox("Invalid / Empty Medicine Code", MsgBoxStyle.Information, "Invalid Code") : Exit Sub
            Else
                txtReference.Focus()
            End If
        End If
            ...
            If SaleDataGrid.RowCount > -1 Then
                ...
                For Each OutRow As DataGridViewRow In SaleDataGrid.Rows
                    HoldQuery = "INSERT INTO SALE_DETAIL_HOLD VALUES ('" & _
                                                         ...
                                                         lblUserID.Text.Trim & "',0,0,'" & OutRow.Cells(7).Value.ToString & "','" & OutRow.Cells(8).Value.ToString & "',0,'x'," & SortID & ",'" & OutRow.Cells(9).Value.ToString & "','" & txtReference.Text & "')"
                    ProcessTransaction(HoldQuery)
                    SortID += 1
                Next OutRow
                ...
                If CheckSaleExist("SALE_MASTER_HOLD", lblInvoice.Text.Trim) = False Then
                    HoldQuery = "INSERT INTO SALE_MASTER_HOLD VALUES ('" & _
                                                         ...
                                                         gblPayment & "," & _
                                                         gblRefund & "," & _
                                                         "'HOLD',0,0,0,'" & Format(Date.Parse(lblDate.Text.Trim), "MM/dd/yyyy") & "',0,'BILLED','" & txtReference.Text & "')"
                Else
    ...
    Private Sub lblHold_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles lblHold.DoubleClick
        ...
        Dim GetHoldInfo As String = "SELECT * from SALE_DETAIL_HOLD WHERE SaleID = N'" & gblCP_ID.Trim & "' order by sortitem"
        ExecuteAdapterQuery(HoldDataAdapter, GetHoldInfo, Connection)
        Dim HoldInfoReader As SqlDataReader = HoldDataAdapter.SelectCommand.ExecuteReader
        ...
        If IsDBNull(ReleaseRec) = False Then
            While ReleaseRec.Read
                ...
                txtReference.Text = ReleaseRec.Item("REFER").ToString
            End While
    ...
    Private Sub cmdSaleReturn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSaleReturn.Click
        Dim UPDT_BALANCE As String = String.Empty
        gblNet = Val(lblNetDues.Text.Trim)
        ...
        If SaleMode = True Then
            ...
            ReturnQuery = "INSERT INTO SALE_MASTER VALUES ('" & _
                          ...
                          "0" & "," & _
                          "0" & "," & _
                          "'RETURN',0," & gblPayment & ",0,'" & Format(Date.Parse(lblDate.Text.Trim.ToString), "MM/dd/yyyy") & "',0','RETURN','Self')"
            ...
            If SaleDataGrid.RowCount > -1 Then
                ...
                For Each OutRow As DataGridViewRow In SaleDataGrid.Rows
                    ...
                    Dim ReturnItemQuery = "INSERT INTO SALE_DETAIL VALUES ('" & _
                                                         ...
                                                         lblUserID.Text.Trim & "',0,0,'" & OutRow.Cells(7).Value.ToString & "','" & OutRow.Cells(8).Value.ToString & "',0,'x'," & SortID & ",'" & OutRow.Cells(9).Value.ToString & "',0,'Self')"
            ...
            'Ledger Entries
            'If Val(gblPayment) > 0 And Val(lblCustID) < 1 Then
            '    Call LedgerDrTransaction(2, "SI-" & lblInvoice.Text, "SALE RETURN", "DR", Val(lblOrgPrice.Text))
            '    Call LedgerCrTransaction(1, "SI-" & lblInvoice.Text, "SALE RETURN", "CR", Val(lblNetDues.Text))
            '    If Val(gblDiscount) > 0 Then Call LedgerCrTransaction(3, "SI-" & lblInvoice.Text, "SALE RETURN", "CR", Val(gblDiscount))
            'End If

            ' To update journal transactions if credit customer info given
            'If Val(lblCustID.Text.Trim) > 0 And ReturnItem = False Then
            '    If Val(gblPayment) > 0 Then
            '        Call LedgerDrTransaction(2, "SI-" & lblInvoice.Text, "SALE RETURN", "DR", Val(lblTotalPrice.Text))
            '        Call LedgerCrTransaction(1, "SI-" & lblInvoice.Text, "SALE RETURN", "CR", Val(lblNetDues.Text))
            '        If Val(gblDiscount) > 0 Then Call LedgerCrTransaction(3, "SI-" & lblInvoice.Text, "SALE RETURN", "CR", Val(gblDiscount))
            '    End If

            '    If Val(gblPayment) = 0 Then
            '        Call LedgerDrTransaction(2, "SI-" & lblInvoice.Text, "CREDIT SALE (" & lblCustID.Text & "-" & lblCustName.Text & ")", "DR", Val(lblTotalPrice.Text))
            '        Call LedgerCrTransaction(4, "SI-" & lblInvoice.Text, "CREDIT SALE (" & lblCustID.Text & "-" & lblCustName.Text & ")", "CR", Val(lblNetDues.Text))
            '        If Val(gblDiscount) > 0 Then Call LedgerCrTransaction(3, "SI-" & lblInvoice.Text, "SALE RETURN", "CR", Val(gblDiscount))
            '        UPDT_BALANCE = "update customers set cust_balance = cust_balance - " & Val(lblNetDues.Text) & " where cust_id = " & Val(lblCustID.Text)
            '        ProcessTransaction(UPDT_BALANCE)
            '    End If
            'End If
            cmdNew_Click(sender, e)
            TotalQty = 0
    ...
    Private Sub UpdateStockQuantity()
        ...
        If Updater.HasRows Then
            While Updater.Read
                ...
                ProcessTransaction("update products set prod_unit = prod_stock / prod_qty_per_unit where prod_code = N'" & Updater.Item("StockID").ToString & "'")
            End While
    ...
    Private Sub lblShowDiscount_Validated(ByVal sender As Object, ByVal e As System.EventArgs)
        'Display the value as local currency.
        Me.lblShowDiscount.Text = Decimal.Parse(Me.lblShowDiscount.Text).ToString("c")
    End Sub

    Private Sub lblShowDiscount_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs)
        Dim currency As Decimal
    ...
    Private Sub CashSale(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        SaveQuery = "INSERT INTO SALE_MASTER VALUES ('" & _
                                             ...
                                             gblPayment & "," & _
                                             gblRefund & "," & _
                                             "'CASH',0,0,0,'" & Format(Date.Parse(lblDate.Text.Trim), "MM/dd/yyyy") & "',0,'BILLED','" & txtReference.Text & "')"
        ...
        If SaleDataGrid.RowCount > -1 Then
            ...
            For Each OutRow As DataGridViewRow In SaleDataGrid.Rows
                Dim SaveItemQuery As String = "INSERT INTO SALE_DETAIL VALUES ('" & _
                                                     ...
                                                     lblUserID.Text.Trim & "',0,0,'" & OutRow.Cells(7).Value.ToString & "','" & OutRow.Cells(8).Value.ToString & "',0,'x'," & SortID & ",'" & OutRow.Cells(9).Value.ToString & "',0,'" & txtReference.Text & "')"
        ...
        'Cash Debit Entry
        SortID = 0
        'Call LedgerDrTransaction(1, "SI-" & lblInvoice.Text, "CASH SALE", "DR", Val(gblPayment))
        'If Val(gblDiscount) > 0 Then Call LedgerDrTransaction(3, "SI-" & lblInvoice.Text, "SALE DISCOUNT", "DR", Val(gblDiscount))
        'Call LedgerCrTransaction(2, "SI-" & lblInvoice.Text, "CASH SALE", "CR", Val(lblTotalPrice.Text))
...
#End Region
    ...
    Private Sub txtReference_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtReference.Enter
        txtReference.Enabled = True
        txtReference.Text = txtReference.Text
    End Sub

    Private Sub txtReference_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtReference.GotFocus
        txtReference.Enabled = True
        txtReference.Text = txtReference.Text
    End Sub

    Private Sub txtReference_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs)

        txtReference.Enabled = False
    End Sub

    Private Sub txtReference_KeyUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtReference.KeyUp
        If e.KeyCode = Keys.F2 Then
            ReferCode.ShowDialog(Me)
            If Len(gblCP_ID) > 0 Then txtReference.Text = gblCP_ID Else txtReference.Text = "Self"
            txtReference.Focus()
        End If
    End Sub
End Class