...
Public Class WholeSalerPayments
    ...
    Private Sub ViewCompanyLedgers_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        LedgerQuery = "SELECT * FROM LEDGERS WHERE LEDG_DATE >= CONVERT(DATETIME,'" & Format(DateTime.Parse(FromDate.Text.Trim.ToString), "yyyy-MM-dd") & " 00:00:00',102) AND " & _
                                      "LEDG_DATE <= CONVERT(DATETIME,'" & Format(DateTime.Parse(ToDate.Text.Trim.ToString), "yyyy-MM-dd") & " 00:00:00',102) " & _
                                      "ORDER BY LEDG_TRAN_ID"
    ...
    Private Sub View_Ledgers(ByVal StandardQuery As String)
        Try
            ...
            PurchaseAdapter.Fill(Me.PaymentDataSet, "LEDGERS")
    ...
    Private Sub cmdPreview_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPreview.Click
        If BindingNavigatorTextBox.Text = String.Empty Then
            LedgerQuery = "SELECT * FROM LEDGERS WHERE LEDG_DATE >= CONVERT(DATETIME,'" & Format(DateTime.Parse(FromDate.Text.Trim.ToString), "yyyy-MM-dd") & " 00:00:00',102) AND " & _
                                      "LEDG_DATE <= CONVERT(DATETIME,'" & Format(DateTime.Parse(ToDate.Text.Trim.ToString), "yyyy-MM-dd") & " 00:00:00',102) " & _
                                      "ORDER BY LEDG_TRAN_ID"
        Else
            LedgerQuery = "SELECT * FROM LEDGERS WHERE LEDG_DATE >= CONVERT(DATETIME,'" & Format(DateTime.Parse(FromDate.Text.Trim.ToString), "yyyy-MM-dd") & " 00:00:00',102) AND " & _
                                      "LEDG_DATE <= CONVERT(DATETIME,'" & Format(DateTime.Parse(ToDate.Text.Trim.ToString), "yyyy-MM-dd") & " 00:00:00',102) AND ACC_NO = N'" & BindingNavigatorTextBox.Text.Trim & "' " & _
                                      "ORDER BY LEDG_TRAN_ID"
        End If
        Call View_Ledgers(LedgerQuery)
    End Sub
