...
Public Class frmCopyRecord
    ...
    Private Sub OK_Button_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OK_Button.Click
        ...
        If response = MsgBoxResult.Yes Then
            ...
            Call GetConnectionString()
            ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, ST_Patient, "PATIENT_ST", Connection)
            Connection.Close()
            Dim ST_Receipt As String = "SELECT * INTO RECEIPT_ST_" & cmbYear.Text.Trim & " FROM RECEIPT_ST"
            ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, ST_Receipt, "RECEIPT_ST", Connection)
            Connection.Close()
            ...
            Call GetConnectionString()
            ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, GX_Patient, "PATIENT_GX", Connection)
            Connection.Close()
            Dim GX_Receipt As String = "SELECT * INTO RECEIPT_GX_" & cmbYear.Text.Trim & " FROM RECEIPT_GX"
            ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, GX_Receipt, "RECEIPT_GX", Connection)
            Connection.Close()
            ...
            'FOR NATINAL RECORD
            Dim National_Patient As String = "SELECT * INTO PATIENT_NATIONAL_" & cmbYear.Text.Trim & " FROM PATIENT_NATIONAL"
            Call GetConnectionString()
            ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, National_Patient, "PATIENT_NATIONAL", Connection)
            Connection.Close()
            Dim National_Receipt As String = "SELECT * INTO RECEIPT_NATIONAL_" & cmbYear.Text.Trim & " FROM RECEIPT_NATIONAL"
            ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, National_Receipt, "RECEIPT_NATIONAL", Connection)
            Connection.Close()
            Dim CommandNP As String = "DELETE FROM PATIENT_NATIONAL"
            ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, CommandNP, "PATIENT_NATIONAL", Connection)
            Connection.Close()
            Dim CommandNR As String = "DELETE FROM RECEIPT_NATIONAL"
            ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, CommandNR, "RECEIPT_NATIONAL", Connection)
            Connection.Close()
