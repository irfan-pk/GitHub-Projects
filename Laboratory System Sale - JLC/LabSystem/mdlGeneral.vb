...
Module mdlGeneral
    ...
    Public Sub ExecuteSqlQuery(ByVal Obj As SqlDataAdapter, ByVal ObjDataSet As DataSet, ByVal MyQuery As String, ByVal TableName As String, ByVal SqlConn As SqlConnection)
        ...
        '// OLD CONNECTION STRING ('SqlConn = New SqlConnection("Data Source=" & ServerName & ";Initial Catalog=JLC;NETWORK LIBRARY=DBMSSOCN;Integrated Security=SSPI;"))
    ...
    Public Function GenerateMax_ID(ByVal tblName As String, ByVal strField As String) As Integer
        ...
        Using Conn As New SqlConnection("Data Source=" & ServerName & ";Initial Catalog=JLC;NETWORK LIBRARY=DBMSSOCN;Integrated Security=SSPI;")
    ...
    Public Sub Print_Slips_Manually()
        ...
        Try
            ...
            If gblReportOption.ToUpper = "GXRECEIPT" Then
                CommandText = "Select * from PATIENT_GX WHERE p_num = " & Val(gblFind.Trim)
                ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, CommandText, "PATIENT_GX", Connection)
            End If

            If gblReportOption.ToUpper = "NPRECEIPT" Then
                CommandText = "Select * from PATIENT_NATIONAL WHERE p_num = " & Val(gblFind.Trim)
                ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, CommandText, "PATIENT_NATIONAL", Connection)
            End If
            ...
            If gblReportOption.ToUpper = "GXRECEIPT" Then objReport.SetDataSource(SqlDataSet.Tables("patient_gx"))
            If gblReportOption.ToUpper = "NPRECEIPT" Then objReport.SetDataSource(SqlDataSet.Tables("patient_national"))
            ...
            With objReport
                ...
                If gblReportOption.ToUpper = "GXRECEIPT" Then
                    .RecordSelectionFormula = "{patient_gx.p_num}=" & Val(gblFind)
                    '' For sent Report directly to Printer
                    frmPrintDialog.Cursor = Cursors.WaitCursor
                    objReport.PrintToPrinter(1, False, 0, 0)
                    '// For export report in pdf format if required uncommit the line
                ...
                If gblReportOption.ToUpper = "NPRECEIPT" Then
                    .RecordSelectionFormula = "{patient_national.p_num}=" & Val(gblFind)
                    '' For sent Report directly to Printer
                    frmPrintDialog.Cursor = Cursors.WaitCursor
                    objReport.PrintToPrinter(1, False, 0, 0)
                    '// For export report in pdf format if required uncommit the line
    ...
    Public Sub GetConnectionString()
        Connection = New SqlConnection("Data Source=SQLNCLI10;Integrated Security=SSPI;Persist Security Info=False;User ID=sa;Password=" & ServerPwd & ";Data Source=" & ServerName & ";DATABASE=JLC;")
    End Sub
