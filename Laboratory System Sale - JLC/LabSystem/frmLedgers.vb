...
#Region "JOURNAL ENTRIES PROCEDURES"
    ...
    Private Sub Show_Journal_Transactions()
        ...
        ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, "SELECT * FROM LEDGER_TRANSACTIONS WHERE LEDG_DATE < CONVERT(DATETIME,'" & Format(DateTime.Parse(dtpFrom.Value.ToShortDateString.ToString), "yyyy-MM-dd") & " 00:00:00', 102) AND (LEDG_ACC_ID = " & Acc_ID & ") ORDER BY LEDG_DATE, LEDG_TRAN_ID", "LEDGER_TRANSACTIONS", Connection)
        ...
        If OpnDr > OpnCr Then
            ...
            lblOpnDebit.Text = Format(Val(OpnDr) - Val(OpnCr), "#.##")
            lblDebit.Text = Format(Val(lblDebit.Text) + Val(lblOpnDebit.Text), "#.##")
            lblOpnCr.Visible = False
            lblOpnCredit.Visible = False
        Else
            lblOpnCr.Visible = True
            lblOpnCredit.Visible = True
            lblOpnCredit.Text = Format(Val(OpnCr) - Val(OpnDr), "#.##")
            lblCredit.Text = Format(Val(lblCredit.Text) + Val(lblOpnCredit.Text), "#.##")
        ...
        ExecuteSqlQuery(SqlDataAdapt, Me.DrDataSet, "SELECT * FROM LEDGER_TRANSACTIONS WHERE LEDG_DATE >= CONVERT(DATETIME,'" & Format(DateTime.Parse(dtpFrom.Value.ToShortDateString.ToString), "yyyy-MM-dd") & " 00:00:00', 102) AND LEDG_DATE <= CONVERT(DATETIME,'" & Format(DateTime.Parse(dtpTo.Value.ToShortDateString.ToString), "yyyy-MM-dd") & " 00:00:00', 102) AND (LEDG_ACC_ID = " & Acc_ID & ") AND (LEDG_DEBIT > 0) ORDER BY LEDG_DATE, LEDG_TRAN_ID", "LEDGER_TRANSACTIONS", Connection)
        Execute_Trans_Reader = SqlDataAdapt.SelectCommand.ExecuteReader
        While Execute_Trans_Reader.Read
            lblDebit.Text = Val(lblDebit.Text) + Convert.ToDouble(Execute_Trans_Reader.Item("LEDG_DEBIT"))
        End While
        dgDebit.DataSource = Me.DrDataSet
        dgDebit.DataMember = "LEDGER_TRANSACTIONS"
        If Not Execute_Trans_Reader Is Nothing Then Execute_Trans_Reader.Close()

        Me.CrDataSet.Clear()
        ExecuteSqlQuery(SqlDataAdapt, Me.CrDataSet, "SELECT * FROM LEDGER_TRANSACTIONS WHERE LEDG_DATE >= CONVERT(DATETIME,'" & Format(DateTime.Parse(dtpFrom.Value.ToShortDateString.ToString), "yyyy-MM-dd") & " 00:00:00', 102) AND LEDG_DATE <= CONVERT(DATETIME,'" & Format(DateTime.Parse(dtpTo.Value.ToShortDateString.ToString), "yyyy-MM-dd") & " 00:00:00', 102) AND (LEDG_ACC_ID = " & Acc_ID & ") AND (LEDG_CREDIT > 0) ORDER BY LEDG_DATE, LEDG_TRAN_ID", "LEDGER_TRANSACTIONS", Connection)
        Execute_Trans_Reader = SqlDataAdapt.SelectCommand.ExecuteReader
        While Execute_Trans_Reader.Read
            lblCredit.Text = Val(lblCredit.Text) + Convert.ToDouble(Execute_Trans_Reader.Item("LEDG_CREDIT"))
        End While
        dgCredit.DataSource = Me.CrDataSet
        dgCredit.DataMember = "LEDGER_TRANSACTIONS"
        If Not Execute_Trans_Reader Is Nothing Then Execute_Trans_Reader.Close()
        ...
        If Val(lblDebit.Text) > Val(lblCredit.Text) Then
            ...
            txtCloCr.Text = Format(Val(lblDebit.Text) - Val(lblCredit.Text), "#.##")
        Else
            lblCloDr.Visible = True
            txtCloDr.Visible = True
            lblCloCr.Visible = False
            txtCloCr.Visible = False
            txtCloDr.Text = Format(Val(lblCredit.Text) - Val(lblDebit.Text), "#.##")
        End If
...
#End Region
    ...
    Private Sub frmLedgers_Resize(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Resize
        If Me.WindowState = FormWindowState.Minimized Then Me.Text = "T - Ledgers" Else Me.Text = My.Application.Info.ProductName
    End Sub
