...
Public Class frmCust_Journal
    ...
    Private Sub frmCust_Journal_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub txtID_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtID.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            If GetName.HasRows Then
                While GetName.Read
                    txtName.Text = GetName.Item("CUST_NAME")
                End While
    ...
    Private Sub Popup_Data()
        Try
            ...
            Dim CommandText As String = "SELECT * FROM CUSTOMERS_PAYM WHERE CUST_CODE = N'" & Val(gblCP_ID) & "' ORDER BY CUST_TR_ID"
            ExecuteAdapterQuery(SqlDataAdapter, CommandText, Connection)
    ...
    Private Sub Updt_General_Journal(ByVal PayType As String)
        ...
        If PayType.ToUpper.Trim = "DEBIT" Then
            ...
            cmdText = ("INSERT INTO CUSTOMERS_PAYM VALUES ('" & Supplier_Code & "','" & Format(Date.Parse(TranDate.Value.ToString), "MM/dd/yyyy") & "','" & Val(txtTran.Text.Trim) & "','" & txtName.Text.Trim & "','" & txtDescription.Text.Trim & "'," & Opn_Bal & "," & Val(txtDebit.Text.Trim) & ",0," & Clo_Bal & ",'" & Format(Date.Parse(TranDate.Value.ToString), "MM/dd/yyyy") & "')")
        End If
        If PayType.ToUpper.Trim = "CREDIT" Then
            Amount = Val(txtCredit.Text.Trim)
            Clo_Bal = Opn_Bal + Val(txtCredit.Text.Trim)
            cmdText = ("INSERT INTO CUSTOMERS_PAYM VALUES ('" & Supplier_Code & "','" & Format(Date.Parse(TranDate.Value.ToString), "MM/dd/yyyy") & "','" & Val(txtTran.Text.Trim) & "','" & txtName.Text.Trim.ToUpper & "','" & StrConv(txtDescription.Text.Trim, VbStrConv.ProperCase) & "'," & Opn_Bal & ",0," & Val(txtCredit.Text.Trim) & "," & Clo_Bal & ",'" & Format(Date.Parse(TranDate.Value.ToString), "MM/dd/yyyy") & "')")
        End If
