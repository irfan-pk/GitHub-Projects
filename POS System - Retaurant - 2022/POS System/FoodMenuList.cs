...
namespace POS_System
{
    ...
    /// Food Menu List
    /// </summary>
    public partial class FoodMenuList
    {
        private bool inputStatus;
        private bool hasDecimal;
        private string ItemName, ItemQty, ItemCode, ItemPrice, ItemPack;
        ...
        {
            if ((lblDisplay.Text.Length) == 0 || (lblDisplay.Text.Trim() == string.Empty))
            {
                MessageBox.Show("Plz. give quantity before select item.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                lblDisplay.Focus();
                return;
            }
            Button btn = (Button)sender;
            ...
            if (ItemReader.HasRows)
            {
                while (ItemReader.Read())
                {
                    ...
                    ItemPrice = ItemReader["PROD_SPRICE"].ToString().Trim();
                    ItemCode = ItemReader["PROD_CD"].ToString().Trim();
                    ItemPack = ItemReader["PROD_PACK"].ToString().Trim();
                    ...
                    if (lvOrderList.Items.Count > 0)
                    {
                        ...
                        {
                            xItem.SubItems[1].Text = (Conversion.Val(lblDisplay.Text.Trim())).ToString();
                            break;
                        }
                    }
                    var withBlock = lvOrderList.Items.Add(ItemName.ToUpper());
                    withBlock.SubItems.Add(lblDisplay.Text.Trim());
                    withBlock.SubItems.Add(ItemPrice.Trim());
                    withBlock.SubItems.Add(ItemPack);
        ...
        private void cmdOrder_Click(object sender, EventArgs e)
        {
            if (My.MyProject.Forms.Parcel.Visible == true)
            {
                if ((lvOrderList.Items.Count ) > 0)
                {
                    var mainForm = Application.OpenForms.OfType<Parcel>().Single();
                    for (int i = 0; i < lvOrderList.Items.Count; i++)
                    {
                        mainForm.ItemName = lvOrderList.Items[i].SubItems[0].Text.Trim();
                        mainForm.Weight = lvOrderList.Items[i].SubItems[1].Text.Trim();
                        mainForm.ItemPrice = lvOrderList.Items[i].SubItems[2].Text.Trim();
                        mainForm.ItemCode = lvOrderList.Items[i].SubItems[3].Text.Trim();
                        mainForm.Handles_SettleOrder();
                        
                    }
                    mainForm.CalculateTotals();
                    mainForm.SaleMode = true;
                    Dispose();
                }
                else
                {
