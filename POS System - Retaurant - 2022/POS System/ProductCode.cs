...
namespace POS_System
{
    ...
    public partial class ProductCode
    {
        ...
        private void frmProductCode_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    string CommandText = "SELECT * FROM PRODUCTS WHERE PROD_DESC = N'" + Strings.Trim(comboBox1.Text.Trim()) + "'";
                    mdlGeneral.ExecuteDatasetQuery(mdlGeneral.ProductCodeAdapter, mdlGeneral.ProductCodeDataset, CommandText, "PRODUCTS", mdlGeneral.Connection);
                    mdlGeneral.gblItemCode = mdlGeneral.ProductCodeDataset.Tables["PRODUCTS"].Rows[0]["PROD_CD"].ToString().Trim();
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            string CommandText = "SELECT * FROM PRODUCTS ORDER BY PROD_DESC";
            ...
            {
                mdlGeneral.gblMessage = "Please add Items first in the table.";
            ...
            while (Reader.Read())
                dtTest.Rows.Add(Reader["prod_cd".ToString().Trim()], Reader["prod_desc"].ToString().Trim());
            if (Reader is null)
                Reader.Close();
