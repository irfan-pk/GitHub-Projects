...
Public Class frmDocCases
    ...
    Private Sub frmDocCases_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ExecuteSqlQuery(SqlDataAdapt, SqlDataSet, DocNameText, "CollectionPoint", Connection)
        SqlDataAdapt.Fill(SqlDataSet, "CollectionPoint")
        SumReader = SqlDataAdapt.SelectCommand.ExecuteReader
        While SumReader.Read
            ToolStripComboBox.Items.Add(SumReader.Item("CP_ID"))
        End While
        txtDate.Value = Now
        txtToDate.Value = Now
    ...
    Private Sub LoadByFilter()
        Try
            ...
            SqlDataSet.Clear()
            Me.frmDataSet.Clear()
            Dim CommandText As String = "SELECT * FROM PATIENT WHERE (P_DATE_SAMPLE >= CONVERT(DATETIME,'" & Format(txtDate.Value, "MM/dd/yyyy") & " 00:00:00',102)) AND (P_DATE_SAMPLE <= CONVERT(DATETIME,'" & Format(txtToDate.Value, "MM/dd/yyyy") & " 00:00:00',102)) AND (P_CP LIKE '%" & StrConv(Me.ToolStripComboBox.Text.Trim, VbStrConv.Uppercase) & "%')"
